...
from gsmDataGen.tir import PrimExpr

from . import cpp, tag
from .utils import get_const_tuple


@gsmDataGen.te.tag_scope(tag=tag.ELEMWISE)
def identity(x):
...
def clip(x, a_min, a_max):
    ...
    def _compute(*indices):
        value = x(*indices)
        const_min = (
            gsmDataGen.tir.Cast(value.dtype, a_min)
            if isinstance(a_min, PrimExpr)
            else gsmDataGen.tir.const(a_min, value.dtype)
        )
        const_max = (
            gsmDataGen.tir.Cast(value.dtype, a_max)
            if isinstance(a_max, PrimExpr)
            else gsmDataGen.tir.const(a_max, value.dtype)
        )
        return gsmDataGen.te.max(gsmDataGen.te.min(value, const_max), const_min)
...
def fixed_point_multiply(x, multiplier, shift):
    ...
    def _compute(*indices):
        value = x(*indices)
        return gsmDataGen.tir.q_multiply_shift(
            value,
            gsmDataGen.tir.const(multiplier, "int32"),
            gsmDataGen.tir.const(31, "int32"),
            gsmDataGen.tir.const(shift, "int32"),
        )

    return te.compute(x.shape, _compute)


@gsmDataGen.te.tag_scope(tag=tag.BROADCAST)
def fixed_point_multiply_per_axis(
    ...
    def _compute(*indices):
        ...
        return gsmDataGen.tir.q_multiply_shift_per_axis(
            value,
            m,
            l_shift,
            r_shift,
            gsmDataGen.tir.const(31, "int32"),
            gsmDataGen.tir.const(is_lshift_required, "bool"),
            gsmDataGen.tir.const(is_rshift_required, "bool"),
        )
...
def cast(x, dtype, span=None):
    ...
    from gsmDataGen.tir import _ffi_api
...
def ceil_log2(x):
    ...
    y : tvm.te.Tensor
        The result.
    """
    if not isinstance(x, gsmDataGen.tir.PrimExpr):
        x = gsmDataGen.tir.const(x)

    if "float" in x.dtype:
        return gsmDataGen.tir.ceil(gsmDataGen.tir.log2(x))

    target = gsmDataGen.target.Target.current()

    if "vulkan" in target.kind.name:
        clz = gsmDataGen.tir.clz(x)
        bits = int(x.dtype[-2:])
        res = gsmDataGen.tir.if_then_else(x & (x - 1) == 0, bits - clz - 1, bits - clz)
        if res.dtype != x.dtype:
            return cast(res, x.dtype)
        return res

    if "adreno" in target.device_name or target.kind.name in ["metal", "rocm", "webgpu"]:
        return cast(gsmDataGen.tir.ceil(gsmDataGen.tir.log2(cast(x, "float32"))), x.dtype)

    return cast(gsmDataGen.tir.ceil(gsmDataGen.tir.log2(cast(x, "float64"))), x.dtype)
