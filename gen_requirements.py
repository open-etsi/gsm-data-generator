...
"""DATAGEN Python requirements.txt generator.

This script generates a set of requirements.txt files (stored in `./requirements`) that describe
DATAGEN's Python dependencies.

## Pieces

DATAGEN can be roughly broken into these named pieces along the lines of Python dependencies:
...
- Extra features (i.e. XGBoost in AutoDATAGEN). These enhance DATAGEN's functionality, but aren't required
  for basic operation.
...
From these pieces, this tool builds:
 ...
 - requirements/dev.txt - Python dependencies needed to develop DATAGEN, such as lint and test tools.
...
def join_requirements() -> typing.Dict[str, typing.Tuple[str, typing.List[str]]]:
    ...
    to_return["all-prod"] = (
        "Combined dependencies for all DATAGEN pieces, excluding dev",
