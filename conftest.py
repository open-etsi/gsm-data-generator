...
#     if not all(k in os.environ for k in ["CI", "DATAGEN_NUM_SHARDS", "DATAGEN_SHARD_INDEX"]):
#         # Only apportion tests if in CI and in a job that is set up for it
#         return

#     num_shards = int(os.environ["DATAGEN_NUM_SHARDS"])
#     shard_index = int(os.environ["DATAGEN_SHARD_INDEX"])
