...
from gsm_data_generator.runtime import Device, Object, PackedFunc
from gsm_data_generator.runtime.profiling import Report
...
class VirtualMachine(object):
    ...
    def __init__(
        self,
        rt_mod: Union[gsm_data_generator.runtime.Module, gsm_data_generator.runtime.Executable],
        ...
        """
        if not isinstance(rt_mod, gsm_data_generator.runtime.Module):
            if isinstance(rt_mod, gsm_data_generator.runtime.Executable):
                rt_mod = rt_mod.jit()
            else:
    ...
    def _setup_device(self, dev: Device, memory_cfg: Union[str, Dict[Device, str]]) -> None:
        ...
        if not isinstance(dev, (list, tuple)):
            if not isinstance(dev, gsm_data_generator.runtime.Device):
                raise TypeError("dev is expected to be Device or List[Device]")
            devs = [dev]

        # CPU is required for executing shape functions
        if devs[-1].device_type % RPC_SESS_MASK != gsm_data_generator.cpu().device_type:
            devs.append(gsm_data_generator.cpu())
    ...
    def _convert(self, arg: Any, cargs: List) -> None:
        ...
        if isinstance(arg, Object):
            ...
            nd_arr = gsm_data_generator.nd.array(arg, device=gsm_data_generator.cpu(0))
            cargs.append(nd_arr)
        elif isinstance(arg, gsm_data_generator.runtime.NDArray):
            cargs.append(arg)
            ...
            dtype = _gettype(arg)
            value = gsm_data_generator.nd.array(np.array(arg, dtype=dtype), device=gsm_data_generator.cpu(0))
            cargs.append(value)
    ...
    def get_outputs(self, func_name: str) -> Union[gsm_data_generator.Object, Tuple[Any]]:
        """
    ...
    def set_instrument(self, instrument: gsm_data_generator.runtime.PackedFunc) -> None:
    ...
    def time_evaluator(
        ...
        f_preproc: str = "",
    ) -> Callable[..., gsm_data_generator.runtime.module.BenchmarkResult]:
        """
