...
def build(
    ...
    pipeline: Optional[Union[str, gsm_data_generator.transform.Pass]] = "default",
):
    """
    ...
    return gsm_data_generator.tir.build(mod, target, pipeline)
...
def _contains_relax(mod: Union[PrimFunc, IRModule]) -> bool:
    ...
    if isinstance(mod, IRModule):
        return any(isinstance(func, gsm_data_generator.relax.Function) for _, func in mod.functions_items())
...
def compile(  # pylint: disable=redefined-builtin
    ...
    *,
    relax_pipeline: Optional[Union[gsm_data_generator.transform.Pass, Callable, str]] = "default",
    tir_pipeline: Optional[Union[gsm_data_generator.transform.Pass, Callable, str]] = "default",
) -> Executable:
    """
    ...
    if _contains_relax(mod):
        return gsm_data_generator.relax.build(
            mod,
            target,
            relax_pipeline=relax_pipeline,
            tir_pipeline=tir_pipeline,
        )
    lib = gsm_data_generator.tir.build(mod, target, pipeline=tir_pipeline)
