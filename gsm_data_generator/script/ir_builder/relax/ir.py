...
from gsm_data_generator import DataType, relax
from gsm_data_generator.ir import PrimExpr, VDevice, IRModule
from gsm_data_generator.relax import (
    Call,
    Expr,
    ExternFunc,
    ShapeExpr,
...
from gsm_data_generator.relax.dpl import PatternMatchingRewriter

############################### Operators ###############################
from gsm_data_generator.relax.op import (
    abs,
    acos,
    acosh,
    add,
...
from gsm_data_generator.relax.op.builtin import stop_lift_params
from gsm_data_generator.relax.struct_info import StructInfo
from gsm_data_generator.relax.utils import args_converter, gen_call_tir_inputs
from gsm_data_generator.runtime import Object as tvm_Object
from gsm_data_generator.runtime import ObjectGeneric
from gsm_data_generator.runtime.ndarray import (
    cpu,
    cuda,
    device,
    ext_dev,
...
def to_vdevice(data: Expr, dst_vdevice: Union[py_str, VDevice]) -> Expr:
    ...
    return gsm_data_generator.relax.op.to_vdevice(data, dst_vdevice)
...
def rewriter(rewriter_mod: Union[IRModule, Type]) -> PatternMatchingRewriter:
    ...
    if not isinstance(rewriter_mod, IRModule):
        rewriter_mod = gsm_data_generator.script.ir_module(rewriter_mod)
...
def call_packed(
    ...
    *args : Expr
    ...
    if kwargs or not is_default:
        attrs = gsm_data_generator.ir.attrs.make_node(attrs_type_key, **kwargs)
