...
def _default_globals() -> Dict[str, Any]:
    from gsm_data_generator.script.parser import ir  # pylint: disable=import-outside-toplevel
    from gsm_data_generator.script.parser import relax  # pylint: disable=import-outside-toplevel
    from gsm_data_generator.script.parser import tir  # pylint: disable=import-outside-toplevel

    extra_vars = {
        "tvm": gsm_data_generator,
        "I": ir,
        "ir": ir,
        "T": tir,
        "tir": tir,
...
def parse(
    ...
    if check_well_formed:
        ...
        if isinstance(ret, (IRModule, gsm_data_generator.relax.Function)) and not gsm_data_generator.relax.analysis.well_formed(
            ret
        ):
            parser.report_error(source_ast, err=WELL_FORMED_ERROR_MESSAGE)

        try:
            gsm_data_generator.tir.analysis.verify_well_formed(check_ret)
