...
from gsm_data_generator.ir import GlobalVar, PrimType
from gsm_data_generator.tir import Buffer, IterVar, PrimExpr, Var
...
def bind_for_value(self: Parser, node: doc.expr, var_name: str, value: Any) -> Any:
    ...
    res : Any
        The bound value.
    """
    if isinstance(value, (list, tuple, gsm_data_generator.ir.Array)):
        for i, v in enumerate(value):
...
def bind_assign_value(self: Parser, node: doc.expr, var_name: str, value: Any) -> Any:
    ...
    if isinstance(value, T.meta_var):
        ...
        value = gsm_data_generator.runtime.convert(value)
...
def visit_expr_stmt(self: Parser, node: doc.Expr) -> None:
    ...
    if res is None:
        ...
        T.evaluate(gsm_data_generator.tir.const(res))
    elif isinstance(res, gsm_data_generator.relax.Call) and not res.args:
        ...
        T.evaluate(gsm_data_generator.tir.call_tir(res.op))
    elif isinstance(res, str):
        # Ignore docstrings
        pass
    elif isinstance(res, gsm_data_generator.tir.stmt.BufferStore):
...
def visit_if(self: Parser, node: doc.If) -> None:
    ...
    with self.var_table.with_frame():
        ...
        if isinstance(predicate, (PrimExpr, gsm_data_generator.tir.expr.ExprOp)):
...
def visit_return(self: Parser, node: doc.Return) -> None:
    ...
    T.evaluate(gsm_data_generator.tir.ret(value))
...
def visit_tvm_declare_function(self: Parser, node: doc.FunctionDef) -> GlobalVar:
    ...
    func_signature = gsm_data_generator.tir.PrimFunc(arg_annotations, None, ret_type=ret_type)
