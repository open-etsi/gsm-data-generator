...
from gsm_data_generator.ffi import get_global_func, register_func
...
from gsm_data_generator.tir.schedule import Trace
from gsm_data_generator.meta_schedule.utils import remove_build_dir
from gsm_data_generator.meta_schedule.testing.tune_utils import generate_input_data
from gsm_data_generator.tir.tensor_intrin import *  # type: ignore # pylint: disable=wildcard-import,unused-wildcard-import
from gsm_data_generator.testing.utils import strtobool
...
def _parse_args():
    ...
    parsed.target = gsm_data_generator.target.Target(parsed.target)
...
def get_runtime_device(target: Target) -> gsm_data_generator.runtime.Device:
    ...
    """
    if target.kind.name == "llvm":
        return gsm_data_generator.cpu()
    elif target.kind.name == "cuda":
        return gsm_data_generator.cuda()
    else:
...
class OriginalModule:
    ...
    def __eq__(self, __o: "OriginalModule") -> bool:  # type: ignore
        return gsm_data_generator.ir.structural_equal(self.mod, __o.mod)

    def __hash__(self) -> int:
        return gsm_data_generator.ir.structural_hash(self.mod)
...
def initializer() -> None:
    ...
    def default_check_metric(  # pylint: disable=unused-variable,unreachable-code
        lhs: List[gsm_data_generator.nd.NDArray], rhs: List[gsm_data_generator.nd.NDArray]
    ) -> bool:
...
def default_input_generator(  # pylint: disable=unused-variable
    mod: IRModule,
) -> List[gsm_data_generator.nd.NDArray]:
    ...
    inputs = [
        gsm_data_generator.nd.array(generate_input_data(input_shape=arg_info.shape, input_dtype=arg_info.dtype))
        for arg_info in args_info
    ]
    return inputs


def to_numpy(a: List[gsm_data_generator.nd.NDArray]) -> List[np.ndarray]:
...
def to_tvm_ndarray(a: List[np.ndarray]) -> List[gsm_data_generator.nd.NDArray]:
    ...
    return [gsm_data_generator.nd.array(x) for x in a]
...
def make_alloc_arg_and_check(
    ...
    def f_with_args_alloc_argument_common(
        device: gsm_data_generator.runtime.Device,
        ...
        """
        return [[gsm_data_generator.nd.array(arg, device=device) for arg in inputs] for _ in range(alloc_repeat)]

    def f_with_args_run_evaluator_common(
        rt_mod: gsm_data_generator.runtime.Module,
        device: gsm_data_generator.runtime.Device,
...
def local_build_and_run(
    ...
    lib = gsm_data_generator.compile(mod, target=target)
    tvm_inputs = [gsm_data_generator.nd.array(inp, device=device) for inp in inputs]
    device.sync()
