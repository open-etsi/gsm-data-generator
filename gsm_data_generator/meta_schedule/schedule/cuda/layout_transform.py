...
from gsm_data_generator import meta_schedule
from gsm_data_generator.tir.schedule import BlockRV, ExprRV, LoopRV
...
def auto_inline_into(sch: gsm_data_generator.tir.Schedule, start_block: BlockRV) -> BlockRV:
    """
...
def get_max_tile_size() -> int:
    ...
    """
    max_tile_size = 32
    cur_target = gsm_data_generator.target.Target.current()
    if cur_target is not None and hasattr(cur_target, "thread_warp_size"):
        max_tile_size = int(cur_target.thread_warp_size)
    return max_tile_size


@gsm_data_generator.register_func("meta_schedule.cuda.layout_transform")
def cuda_layout_transform_schedule_rule(
    sch: gsm_data_generator.tir.Schedule, block: BlockRV, testing_tile_sizes: Optional[List[int]] = None
) -> List[gsm_data_generator.tir.Schedule]:
    """
