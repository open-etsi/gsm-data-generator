...
import gsm_data_generator.arith
import gsm_data_generator.tir
import gsm_data_generator.te
import gsm_data_generator.ffi

from gsm_data_generator.target import codegen
from gsm_data_generator.contrib import nvcc, cudnn, rocm
import gsm_data_generator.contrib.hexagon._ci_env_check as hexagon
from gsm_data_generator.error import TVMError
import gsm_data_generator.contrib.utils
...
def check_bool_expr_is_true(bool_expr, vranges, cond=None):
    ...
    """
    if cond is not None:
        bool_expr = gsm_data_generator.te.any(gsm_data_generator.tir.Not(cond), bool_expr)
    ...
    def _run_expr(expr, vranges):
        ...
        def _compute_body(*us):
            ...
            return gsm_data_generator.tir.stmt_functor.substitute(expr, vmap)

        A = gsm_data_generator.te.compute([r.extent.value for v, r in vranges.items()], _compute_body)
        args = [gsm_data_generator.nd.empty(A.shape, A.dtype)]
        mod = gsm_data_generator.compile(gsm_data_generator.IRModule.from_expr(gsm_data_generator.te.create_prim_func([A])))
        mod(*args)
        return args[0].numpy()
...
def check_int_constraints_trans_consistency(constraints_trans, vranges=None):
    ...
    def _check_forward(constraints1, constraints2, varmap, backvarmap):
        ...
        cond_on_vars = gsm_data_generator.tir.const(1, "bool")
        for v in constraints1.variables:
            if v in varmap:
                # variable mapping is consistent
                v_back = ana.simplify(gsm_data_generator.tir.stmt_functor.substitute(varmap[v], backvarmap))
                cond_on_vars = gsm_data_generator.te.all(cond_on_vars, v == v_back)
        # Also we have to check that the new relations are true when old relations are true
        cond_subst = gsm_data_generator.tir.stmt_functor.substitute(
            gsm_data_generator.te.all(gsm_data_generator.tir.const(1, "bool"), *constraints2.relations), backvarmap
        )
        ...
        for v in constraints2.variables:
            if v in constraints2.ranges:
                ...
                range_cond = gsm_data_generator.te.all(v >= r.min, v < r.min + r.extent)
                range_cond = gsm_data_generator.tir.stmt_functor.substitute(range_cond, backvarmap)
                cond_subst = gsm_data_generator.te.all(cond_subst, range_cond)
        cond_subst = ana.simplify(cond_subst)
        check_bool_expr_is_true(
            gsm_data_generator.te.all(cond_subst, cond_on_vars),
            all_vranges,
            cond=gsm_data_generator.te.all(gsm_data_generator.tir.const(1, "bool"), *constraints1.relations),
        )
...
def _get_targets(target_names=None):
    ...
    for target in target_names:
        ...
        if target_kind == "cuda" and "cudnn" in gsm_data_generator.target.Target(target).attrs.get("libs", []):
            is_enabled = gsm_data_generator.support.libinfo()["USE_CUDNN"].lower() in ["on", "true", "1"]
            is_runnable = is_enabled and cudnn.exists()
        elif target_kind == "hexagon":
            is_enabled = gsm_data_generator.support.libinfo()["USE_HEXAGON"].lower() in ["on", "true", "1"]
            ...
            is_enabled = gsm_data_generator.runtime.enabled(target_kind)
            is_runnable = is_enabled and gsm_data_generator.device(target_kind).exist
    ...
    if all(not t["is_runnable"] for t in targets):
        if gsm_data_generator.runtime.enabled("llvm"):
            logging.warning(
...
def enabled_targets():
    ...
    """
    return [(t["target"], gsm_data_generator.device(t["target"])) for t in _get_targets() if t["is_runnable"]]
...
class Feature:
    ...
    def _run_only_marks(self):
        ...
        if self.target_kind_hardware is not None:
            yield pytest.mark.skipif(
                not gsm_data_generator.device(self.target_kind_hardware).exist,
...
def _any_gpu_exists():
    return (
        gsm_data_generator.cuda().exist
        or gsm_data_generator.rocm().exist
        or gsm_data_generator.opencl().exist
        or gsm_data_generator.metal().exist
        or gsm_data_generator.vulkan().exist
    )


def _multi_gpu_exists():
    return (
        (gsm_data_generator.cuda(0).exist and gsm_data_generator.cuda(1).exist)
        or (gsm_data_generator.rocm(0).exist and gsm_data_generator.rocm(1).exist)
        or (gsm_data_generator.opencl(0).exist and gsm_data_generator.opencl(1).exist)
        or (gsm_data_generator.metal(0).exist and gsm_data_generator.metal(1).exist)
        or (gsm_data_generator.vulkan(0).exist and gsm_data_generator.vulkan(1).exist)
    )
...
requires_tensorcore = Feature(
    "tensorcore",
    "NVIDIA Tensor Core",
    run_time_check=lambda: gsm_data_generator.cuda().exist and nvcc.have_tensorcore(gsm_data_generator.cuda().compute_version),
...
requires_matrixcore = Feature(
    "matrixcore",
    "AMD Matrix Core",
    run_time_check=lambda: gsm_data_generator.rocm().exist and rocm.have_matrixcore(gsm_data_generator.rocm().compute_version),
...
def _has_cpu_feat(features):
    ...
    has_feat = codegen.target_has_features(features, gsm_data_generator.target.Target(target))
...
def _cmake_flag_enabled(flag):
    flag = gsm_data_generator.support.libinfo()[flag]
...
def requires_cuda_compute_version(major_version, minor_version=0):
    ...
    try:
        ...
        compute_version = gsm_data_generator.contrib.nvcc.parse_compute_version(arch)
    except ValueError:
...
def is_ampere_or_newer():
    ...
    arch = gsm_data_generator.contrib.nvcc.get_target_compute_version()
    major, minor = gsm_data_generator.contrib.nvcc.parse_compute_version(arch)
...
class CompareBeforeAfter:
    ...
    def _normalize_ir_module(cls, func):
        if isinstance(func, (gsm_data_generator.tir.PrimFunc, gsm_data_generator.IRModule)):
            ...
            def inner(self):
                ...
                return gsm_data_generator.script.from_source(source_code, check_well_formed=self.check_well_formed)
    ...
    def _normalize_transform(cls, transform):
        def apply(module_transform):
            def inner(obj):
                if isinstance(obj, gsm_data_generator.IRModule):
                    return module_transform(obj)
                elif isinstance(obj, gsm_data_generator.tir.PrimFunc):
                    mod = gsm_data_generator.IRModule({"main": obj})
        ...
        elif isinstance(transform, gsm_data_generator.ir.transform.Pass):
    ...
    def test_compare(self, before, expected, transform):
        ...
        elif isinstance(expected, (gsm_data_generator.tir.PrimFunc, gsm_data_generator.ir.IRModule)):
            after = transform(before)

            try:
                # overwrite global symbol so it doesn't come up in the comparison
                if isinstance(after, gsm_data_generator.tir.PrimFunc):
