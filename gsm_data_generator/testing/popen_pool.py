...
@gsm_data_generator.ffi.register_func("testing.identity_py")
def identity_py(arg):
    return arg


def register_ffi():
    @gsm_data_generator.ffi.register_func("testing.nested_identity_py")
    def _identity_py(arg):  # pylint: disable=unused-variable
        return arg


def call_py_ffi(arg):
    _identity_py = gsm_data_generator.ffi.get_global_func("testing.nested_identity_py")
    return _identity_py(arg)


def call_cpp_ffi(arg):
    return gsm_data_generator.testing.echo(arg)


def call_cpp_py_ffi(arg):
    return gsm_data_generator.testing.identity_cpp(arg)
