...
import gsm_data_generator.arith._ffi_api
import gsm_data_generator.tir
import gsm_data_generator.tir._ffi_api
from gsm_data_generator.ir import Array
...
def compute(shape, fcompute, name="compute", tag="", attrs=None, varargs_names=None):
    ...
    shape = (shape,) if isinstance(shape, gsm_data_generator.tir.PrimExpr) else shape
    # for python3
    ...
    dim_var = [gsm_data_generator.tir.IterVar((0, s), x, 0) for x, s in zip(arg_names, shape[:out_ndim])]
...
def scan(init, update, state_placeholder, inputs=None, name="scan", tag="", attrs=None):
    ...
    axis = gsm_data_generator.tir.IterVar((init[0].shape[0], update[0].shape[0]), f"{name}.idx", 3)
...
def extern(
    ...
    if _tag.TagScope.get_current() is not None:
        if tag != "":
            raise ValueError("nested tag is not allowed for now")
        tag = _tag.TagScope.get_current().tag
    shape = (shape,) if isinstance(shape, (gsm_data_generator.tir.PrimExpr, _Integral)) else shape
    if shape == () or isinstance(shape[0], (gsm_data_generator.tir.PrimExpr, _Integral)):
        shape = [shape]
    ...
    for t in inputs:
        ...
        if in_buffers is None:
            input_placeholders.append(
                gsm_data_generator.tir.decl_buffer(
                    t.shape, t.dtype, t.op.name, elem_offset=gsm_data_generator.tir.Var("elem_offset", "int32")
                )
            )
        types.add(t.dtype)
    ...
    if out_buffers is None:
        ...
        for shp, dt in zip(shape, dtype):
            output_placeholders.append(
                gsm_data_generator.tir.decl_buffer(shp, dt, name, elem_offset=gsm_data_generator.tir.Var("elem_offset", "int32"))
            )
    body = fcompute(input_placeholders, output_placeholders)
    if isinstance(body, gsm_data_generator.tir.PrimExpr):
        body = gsm_data_generator.tir.Evaluate(body)
    if not isinstance(body, gsm_data_generator.tir.Stmt):
        raise ValueError(
...
def extern_primfunc(input_tensors: List[_tensor.Tensor], primfunc: gsm_data_generator.tir.PrimFunc, **kwargs):
    ...
    dt_access_map = gsm_data_generator.arith._ffi_api.DomainTouchedAccessMap(primfunc)
...
def var(name="tindex", dtype="int32", span=None):
    ...
    """
    return gsm_data_generator.tir.Var(name, dtype, span)
...
def const(value, dtype="int32", span=None):
    ...
    """
    return gsm_data_generator.tir.const(value, dtype, span)
...
def size_var(name="size", dtype="int32", span=None):
    ...
    """
    return gsm_data_generator.tir.SizeVar(name, dtype, span)
...
def thread_axis(dom=None, tag="", name="", span=None):
    ...
    return gsm_data_generator.tir.IterVar(dom, name, 1, tag, span)
...
def reduce_axis(dom, name="rv", thread_tag="", span=None):
    ...
    """
    return gsm_data_generator.tir.IterVar(dom, name, 2, thread_tag, span)


def create_prim_func(
    ops: List[Union[_tensor.Tensor, gsm_data_generator.tir.Var]], index_dtype_override: Optional[str] = None
) -> gsm_data_generator.tir.PrimFunc:
...
AXIS_SEPARATOR = gsm_data_generator.tir.IndexMap.AXIS_SEPARATOR
