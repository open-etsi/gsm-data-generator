...
#     # NB: This will either be the source directory (if DATAGEN is run
#     # inplace) or the install directory (if DATAGEN is installed).
#     # An installed DATAGEN's curr_path will look something like:
#     #   $PREFIX/lib/python3.6/site-packages/gsm_datagen/_ffi
...
#     if os.environ.get("DATAGEN_LIBRARY_PATH", None):
#         dll_path.append(os.environ["DATAGEN_LIBRARY_PATH"])
...
#     # use extra DATAGEN_HOME environment for finding libraries.
#     if os.environ.get("DATAGEN_HOME", None):
#         datagen_source_home_dir = os.environ["DATAGEN_HOME"]
#     else:
#         datagen_source_home_dir = source_dir

#     if os.path.isdir(datagen_source_home_dir):
#         dll_path.append(os.path.join(datagen_source_home_dir, "web", "dist", "wasm"))
#         dll_path.append(os.path.join(datagen_source_home_dir, "web", "dist"))
...
#     """
#     use_runtime = os.environ.get("DATAGEN_USE_RUNTIME_LIB", False)
...
#             lib_dll_names = ["libdatagen.dll", "gsm_datagen.dll"]
#             runtime_dll_names = ["libdatagen_runtime.dll", "datagen_runtime.dll"]
#             ext_lib_dll_names = [
#                 "3rdparty/cutlass_fpA_intB_gemm/cutlass_kernels/libfpA_intB_gemm.dll",
#                 "3rdparty/libflash_attn/src/libflash_attn.dll",
#             ]
#         elif sys.platform.startswith("darwin"):
#             lib_dll_names = ["libdatagen.dylib"]
#             runtime_dll_names = ["libdatagen_runtime.dylib"]
#             ext_lib_dll_names = [
#                 "3rdparty/cutlass_fpA_intB_gemm/cutlass_kernels/libfpA_intB_gemm.dylib",
#                 "3rdparty/libflash_attn/src/libflash_attn.dylib",
#             ]
#         else:
#             lib_dll_names = ["libdatagen.so"]
#             runtime_dll_names = ["libdatagen_runtime.so"]
#             ext_lib_dll_names = [
...
#             for p in dll_path
#             if not p.endswith("python/gsm_datagen")
#         ]
#         runtime_dll_path = [
#             os.path.join(p, name)
...
#     """
#     if os.environ.get("DATAGEN_HOME", None):
#         source_dir = os.environ["DATAGEN_HOME"]
#     else:
...
#     if os.environ.get("DATAGEN_INCLUDE_PATH", None):
#         header_path.append(os.environ["DATAGEN_INCLUDE_PATH"])
...
#     if name is not None:
#         if isinstance(name, list):
#             datagen_include_path = []
#             for n in name:
#                 datagen_include_path += [os.path.join(p, n) for p in header_path]
#         else:
#             datagen_include_path = [os.path.join(p, name) for p in header_path]
#         dlpack_include_path = []
#         dmlc_include_path = []
#     else:
#         datagen_include_path = [os.path.join(p, "include") for p in header_path]
#         datagen_ffi_include_path = [os.path.join(p, "ffi/include") for p in header_path]
...
#         include_found = [p for p in datagen_include_path if os.path.exists(p) and os.path.isdir(p)]
#         include_found += [p for p in datagen_ffi_include_path if os.path.exists(p) and os.path.isdir(p)]
...
#             + "List of candidates:\n"
#             + str("\n".join(datagen_include_path + dlpack_include_path))
#         )
#         if not optional:
#             raise RuntimeError(message)
#         return None
...
# The following line is set by gsm_datagen/python/update_version.py
