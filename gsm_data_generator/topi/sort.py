...
def sort(data, axis=-1, is_ascend=1):
    ...
    out_buf = gsm_data_generator.tir.decl_buffer(data.shape, data.dtype, "out_buf", data_alignment=8)
    out = te.extern(
        data.shape,
        [data],
...
def argsort(data, valid_count=None, axis=-1, is_ascend=1, dtype="float32"):
    ...
    if valid_count is not None:
        valid_count_buf = gsm_data_generator.tir.decl_buffer(
            valid_count.shape, valid_count.dtype, "valid_count_buf", data_alignment=4
        )
        out_buf = gsm_data_generator.tir.decl_buffer(data.shape, "int32", "out_buf", data_alignment=8)
        out = te.extern(
            data.shape,
            [data, valid_count],
            lambda ins, outs: gsm_data_generator.tir.call_packed(
        ...
        out_buf = gsm_data_generator.tir.decl_buffer(data.shape, dtype, "out_buf", data_alignment=8)
        out = te.extern(
            data.shape,
            [data],
            lambda ins, outs: gsm_data_generator.tir.call_packed(
...
def topk(data, k=1, axis=-1, ret_type="both", is_ascend=False, dtype="int64"):
    ...
    kvar = gsm_data_generator.te.size_var("k")
    ...
    if ret_type in ["both", "values"]:
        out_bufs.append(gsm_data_generator.tir.decl_buffer(out_shape, data.dtype, "value_buf", data_alignment=8))
    if ret_type in ["both", "indices"]:
        out_bufs.append(gsm_data_generator.tir.decl_buffer(out_shape, dtype, "indices_buf", data_alignment=8))
