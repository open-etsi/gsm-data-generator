...
from gsm_data_generator.topi.utils import nchw_pack_layout, nchw_xc_layout
from .. import tag
...
def can_convert_multiply_to_intdiv(origin_size, scaled_size):
    ...
    if not isinstance(scaled_size, gsm_data_generator.tir.expr.IntImm):
        return False
...
def get_1d_pixel(data, layout, image_width, n, c, x, cc, ib, ic):
    """Get 1d pixel"""
    x = gsm_data_generator.te.max(gsm_data_generator.te.min(x, image_width - 1), 0)
    if layout == "NWC":
...
def get_2d_pixel(data, layout, image_height, image_width, n, c, y, x, cc, ib, ic):
    """Get 2d pixel"""
    y = gsm_data_generator.te.max(gsm_data_generator.te.min(y, image_height - 1), 0)
    x = gsm_data_generator.te.max(gsm_data_generator.te.min(x, image_width - 1), 0)
    if layout == "NHWC":
...
def get_3d_pixel(data, layout, image_depth, image_height, image_width, n, c, z, y, x, cc):
    """Get 3d pixel"""
    z = gsm_data_generator.te.max(gsm_data_generator.te.min(z, image_depth - 1), 0)
    y = gsm_data_generator.te.max(gsm_data_generator.te.min(y, image_height - 1), 0)
    x = gsm_data_generator.te.max(gsm_data_generator.te.min(x, image_width - 1), 0)
    if layout == "NDHWC":
...
def _resize_1d(
    ...
    if coordinate_transformation_mode == "tf_crop_and_resize":
        ...
        value = gsm_data_generator.tir.if_then_else(
            in_x < 0,
            extrapolation_value,
            gsm_data_generator.tir.if_then_else(in_x > image_width - 1, extrapolation_value, value),
        )
...
def resize1d(
    ...
    for i in range(1):
        if isinstance(size[i], int):
            size[i] = gsm_data_generator.tir.IntImm("int32", size[i])
...
def _resize_2d(
    ...
    if coordinate_transformation_mode == "tf_crop_and_resize":
        out = gsm_data_generator.tir.if_then_else(
            in_y < 0,
            extrapolation_value,
            gsm_data_generator.tir.if_then_else(in_y > image_height - 1, extrapolation_value, value),
        )
        # use extrapolation_value if in_x is out of boundary
        value = gsm_data_generator.tir.if_then_else(
            in_x < 0,
            extrapolation_value,
            gsm_data_generator.tir.if_then_else(in_x > image_width - 1, extrapolation_value, out),
        )
    return _cast_output(value, data.dtype, out_dtype=out_dtype)
...
def _resize_3d(
    ...
    if coordinate_transformation_mode == "tf_crop_and_resize":
        out = gsm_data_generator.tir.if_then_else(
            in_z < 0,
            extrapolation_value,
            gsm_data_generator.tir.if_then_else(in_z > image_depth - 1, extrapolation_value, value),
        )
        out = gsm_data_generator.tir.if_then_else(
            in_y < 0,
            extrapolation_value,
