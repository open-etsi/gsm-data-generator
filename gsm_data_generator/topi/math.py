...
from gsm_data_generator.tir import PrimExpr

from . import cpp, tag
from .utils import get_const_tuple


@gsm_data_generator.te.tag_scope(tag=tag.ELEMWISE)
def identity(x):
...
def clip(x, a_min, a_max):
    ...
    def _compute(*indices):
        value = x(*indices)
        const_min = (
            gsm_data_generator.tir.Cast(value.dtype, a_min)
            if isinstance(a_min, PrimExpr)
            else gsm_data_generator.tir.const(a_min, value.dtype)
        )
        const_max = (
            gsm_data_generator.tir.Cast(value.dtype, a_max)
            if isinstance(a_max, PrimExpr)
            else gsm_data_generator.tir.const(a_max, value.dtype)
        )
        return gsm_data_generator.te.max(gsm_data_generator.te.min(value, const_max), const_min)
...
def fixed_point_multiply(x, multiplier, shift):
    ...
    def _compute(*indices):
        value = x(*indices)
        return gsm_data_generator.tir.q_multiply_shift(
            value,
            gsm_data_generator.tir.const(multiplier, "int32"),
            gsm_data_generator.tir.const(31, "int32"),
            gsm_data_generator.tir.const(shift, "int32"),
        )

    return te.compute(x.shape, _compute)


@gsm_data_generator.te.tag_scope(tag=tag.BROADCAST)
def fixed_point_multiply_per_axis(
    ...
    def _compute(*indices):
        ...
        return gsm_data_generator.tir.q_multiply_shift_per_axis(
            value,
            m,
            l_shift,
            r_shift,
            gsm_data_generator.tir.const(31, "int32"),
            gsm_data_generator.tir.const(is_lshift_required, "bool"),
            gsm_data_generator.tir.const(is_rshift_required, "bool"),
        )
...
def cast(x, dtype, span=None):
    ...
    from gsm_data_generator.tir import _ffi_api
...
def ceil_log2(x):
    ...
    y : tvm.te.Tensor
        The result.
    """
    if not isinstance(x, gsm_data_generator.tir.PrimExpr):
        x = gsm_data_generator.tir.const(x)

    if "float" in x.dtype:
        return gsm_data_generator.tir.ceil(gsm_data_generator.tir.log2(x))

    target = gsm_data_generator.target.Target.current()

    if "vulkan" in target.kind.name:
        clz = gsm_data_generator.tir.clz(x)
        bits = int(x.dtype[-2:])
        res = gsm_data_generator.tir.if_then_else(x & (x - 1) == 0, bits - clz - 1, bits - clz)
        if res.dtype != x.dtype:
            return cast(res, x.dtype)
        return res

    if "adreno" in target.device_name or target.kind.name in ["metal", "rocm", "webgpu"]:
        return cast(gsm_data_generator.tir.ceil(gsm_data_generator.tir.log2(cast(x, "float32"))), x.dtype)

    return cast(gsm_data_generator.tir.ceil(gsm_data_generator.tir.log2(cast(x, "float64"))), x.dtype)
