...
from gsm_data_generator.contrib.thrust import can_use_rocthrust, can_use_thrust
...
def _get_thrust_func_name(tvmop):
    tvmop_to_thrust_func_name = {gsm_data_generator.tir.generic.add: "tvm.contrib.thrust.sum_scan"}
...
def _can_use_scan_thrust(binop):
    ...
    return binop == gsm_data_generator.tir.generic.add and any(
        [
...
def exclusive_scan_ir(data, output, reduction=None, binop=gsm_data_generator.tir.generic.add, identity_value=0):
    ...
    with ib.else_scope():
        ...
        with ib.for_range(0, cast(lim, "int32"), dtype="int32") as l2_width:
            ...
            with ib.new_scope():
                ...
                ib.scope_attr(
                    bx,
                    "thread_extent",
                    gsm_data_generator.tir.generic.cast(ceil_div(scan_axis_size, max_threads * width), "int32"),
                )
                tid = bx * nthread_tx + tx
                ...
                with ib.if_scope(start[0] < scan_axis_size):
                    middle[0] = start[0] + gsm_data_generator.tir.indexdiv(width, 2)
                    end[0] = gsm_data_generator.te.min(start[0] + width, scan_axis_size)
        ...
        with ib.for_range(0, cast(lim, "int32"), dtype="int32") as l2_width:
            ...
            with ib.new_scope():
                ...
                start[0] = width * tid
                with ib.if_scope(gsm_data_generator.tir.all(start[0] < scan_axis_size)):
                    middle[0] = start[0] + gsm_data_generator.tir.indexdiv(width, 2)
                    end[0] = gsm_data_generator.tir.min(start[0] + width, scan_axis_size)
                    with ib.if_scope(middle[0] < scan_axis_size):
...
def get_reduction_from_exclusive_scan(data, ex_scan_output, binop=gsm_data_generator.tir.generic.add):
    ...
    data_buf = gsm_data_generator.tir.decl_buffer(data.shape, data.dtype, "valid_indices_buf", data_alignment=8)
    ex_scan_output_buf = gsm_data_generator.tir.decl_buffer(
...
def scan_thrust(
    ...
    binop=gsm_data_generator.tir.generic.add,
    workspace=None,
):
    ...
    output_buf = gsm_data_generator.tir.decl_buffer(data.shape, output_dtype, "output_buf", data_alignment=8)

    workspace_buf = (
        gsm_data_generator.tir.decl_buffer(workspace.shape, workspace.dtype, "workspace_buf", data_alignment=8)
...
def inclusive_scan(
    data, axis=-1, output_dtype=None, binop=gsm_data_generator.tir.generic.add, identity_value=0, workspace=None
):
...
def scanop(
    data: gsm_data_generator.te.Tensor,
    binop: Callable[["gsm_data_generator.Expr", "gsm_data_generator.Expr"], "gsm_data_generator.Expr"],
    identity_value: Union[float, int],
    axis: Optional[int] = None,
    dtype: Optional[str] = None,
    exclusive: Optional[bool] = None,
    workspace: Optional[gsm_data_generator.te.Tensor] = None,
) -> gsm_data_generator.te.Tensor:
...
def cumprod(
    ...
    return scanop(
        data=data,
        binop=gsm_data_generator.tir.generic.multiply,
        identity_value=1,
        axis=axis,
        dtype=dtype,
        exclusive=exclusive,
