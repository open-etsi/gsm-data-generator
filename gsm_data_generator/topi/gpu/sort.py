...
def _sort_init(ib, shape, axis, keys_in, keys_out, values_out=None, value_init_func=None):
    ...
    # Set up threading
    max_threads = int(gsm_data_generator.target.Target.current(allow_none=False).max_num_threads)
...
def _odd_even_sort(
    ...
    nthread_by = axis_mul_before * axis_mul_after
    with ib.new_scope():
        ib.scope_attr(gsm_data_generator.tir.const(0), "hand_threaded", 0)
        tx, bx, by = _get_threads(ib, nthread_tx, nthread_bx, nthread_by)
        by, bz = by % axis_mul_before, by // axis_mul_before
        tid = 2 * tx
        start = bx * block_size
        ...
        ib.emit(gsm_data_generator.tir.Call(None, "tir.tvm_storage_sync", gsm_data_generator.runtime.convert(["shared"])))

        idxm = gsm_data_generator.tir.indexmod
        # OddEvenTransposeSort
        current_sort_num = gsm_data_generator.tir.min(block_size, size - start)
...
def _sort_common(
    ...
    def get_merge_begin(source, base_idx, aCount, bCount, aStart, bStart, diag, step_count):
        ...
        first[0] = gsm_data_generator.te.max(0, diag - bCount)
        last[0] = gsm_data_generator.te.min(diag, aCount)
    ...
    def serial_merge(
        ...
        with ib.for_range(0, gsm_data_generator.te.min(aCount + bCount - diag, step_count)) as count:
            i_idx = base_idx + i[0]
            j_idx = base_idx + j[0]
            ...
            with ib.if_scope(gsm_data_generator.tir.all(i[0] < aStart + aCount, j[0] < bStart + bCount)):
    ...
    with ib.for_range(0, cast(upper_lim - lower_lim, "int64"), dtype="int64") as l2_width:
        ...
        with ib.new_scope():
            target = gsm_data_generator.target.Target.current()
            if "vulkan" in str(target):
                ...
                ntx = max_threads
                nbx = gsm_data_generator.tir.generic.cast(ceil_div(width, max_threads * thread_work), "int32")
                nbz = gsm_data_generator.tir.generic.cast(ceil_div(size, width), "int32")
                tx, bx, by = _get_threads(ib, ntx, nbx, nthread_by * nbz)
            else:
                ntx = gsm_data_generator.tir.generic.cast(gsm_data_generator.te.min(max_threads, width), "int32")
                nbx = gsm_data_generator.tir.generic.cast(ceil_div(width, max_threads * thread_work), "int32")
                nbz = gsm_data_generator.tir.generic.cast(ceil_div(size, width), "int32")
                tx, bx, by = _get_threads(ib, ntx, nbx, nthread_by * nbz)
            ...
            def mergesort(source, dest, source_idx, dest_idx, size, width, even):
                ...
                start = width * bz
                middle = cast(gsm_data_generator.te.min(start + gsm_data_generator.tir.indexdiv(width, 2), size), "int64")
                end = cast(gsm_data_generator.te.min(start + width, size), "int64")
                with ib.if_scope(start < size):
                    ...
                    with ib.else_scope():
                        ...
                        def do_merge(first, last):
                            ...
                            aCount = gsm_data_generator.te.min(middle - aStart, step_count)
                            bCount = gsm_data_generator.te.min(end - bStart, step_count)
                            mergepath(
                                source,
                                dest,
                                source_idx,
            ...
            mergesort(
                ...
                size,
                width,
                gsm_data_generator.tir.indexmod(l2_width, 2) == 0,
            )
    nthread_by = axis_mul_before * axis_mul_after
    nthread_tx = max_threads
    nthread_bx = ceil_div(size, nthread_tx)
    ...
    with ib.if_scope(
        gsm_data_generator.tir.all(upper_lim > lower_lim, gsm_data_generator.tir.indexmod(upper_lim - lower_lim, 2) == 1)
    ):
        with ib.new_scope():
            tx, bx, by = _get_threads(ib, nthread_tx, nthread_bx, nthread_by)
            tid = bx * nthread_tx + tx
...
def sort_ir(
    ...
    with ib.if_scope(shape[axis] > 0):
        axis_mul_before, axis_mul_after = _sort_init(
            ...
            values_out,
            indices_out,
            value_init_func=lambda _, tid: gsm_data_generator.tir.generic.cast(tid, indices_out.dtype),
        )
...
def sort(data, axis=-1, is_ascend=1):
    ...
    value_buf = gsm_data_generator.tir.decl_buffer(data.shape, data.dtype, "value_buf", data_alignment=8)
    value_buf_swap = gsm_data_generator.tir.decl_buffer(data.shape, data.dtype, "value_buf_swap", data_alignment=8)
...
def sort_thrust(data, axis=-1, is_ascend=1, workspace=None):
    ...
    value_buf = gsm_data_generator.tir.decl_buffer(data.shape, data.dtype, "value_buf", data_alignment=8)
    indices_buf = gsm_data_generator.tir.decl_buffer(data.shape, dtype, "out_buf", data_alignment=8)

    def f_compute(ins, outs):
        args = ["tvm.contrib.thrust.sort", ins[0], outs[0], outs[1], is_ascend]
        if workspace is not None:
            args.append(ins[1])
        return gsm_data_generator.tir.call_packed(*args)
...
def argsort(data, axis=-1, is_ascend=1, dtype="float32", ret_type="indices"):
    ...
    value_buf = gsm_data_generator.tir.decl_buffer(data.shape, data.dtype, "value_buf", data_alignment=8)
    value_swap_buf = gsm_data_generator.tir.decl_buffer(data.shape, data.dtype, "value_swap_buf", data_alignment=8)
    indices_buf = gsm_data_generator.tir.decl_buffer(data.shape, dtype, "out_buf", data_alignment=8)
    indices_swap_buf = gsm_data_generator.tir.decl_buffer(data.shape, dtype, "out_swap_buf", data_alignment=8)
...
def topk(data, k=1, axis=-1, ret_type="both", is_ascend=False, dtype="int64"):
    ...
    values_buf = gsm_data_generator.tir.decl_buffer(data.shape, data.dtype, "values_buf", data_alignment=8)
    values_swap_buf = gsm_data_generator.tir.decl_buffer(
        data.shape, data.dtype, "values_swap_buf", data_alignment=8
    )
    indices_buf = gsm_data_generator.tir.decl_buffer(data.shape, dtype, "indices_buf", data_alignment=8)
    indices_swap_buf = gsm_data_generator.tir.decl_buffer(data.shape, dtype, "indies_swap_buf", data_alignment=8)
    ...
    for i in range(ndim):
        if i == axis:
            end.append(k if isinstance(k, int) else gsm_data_generator.te.size_var("dim"))
        else:
            end.append(dshape[i])
    if ret_type == "both":
...
def topk_thrust(
    ...
    data_buf = gsm_data_generator.tir.decl_buffer(data.shape, data.dtype, "data_buf", data_alignment=8)
    if workspace is not None:
        workspace_buf = gsm_data_generator.tir.decl_buffer(
            workspace.shape, workspace.dtype, "workspace_buf", data_alignment=8
        )
    else:
        workspace_buf = None
    out_bufs = [
        gsm_data_generator.tir.decl_buffer(data.shape, data.dtype, "value_buf", data_alignment=8),
        gsm_data_generator.tir.decl_buffer(data.shape, dtype, "indices_buf", data_alignment=8),
    ]
    ...
    if isinstance(k, gsm_data_generator.tir.IntImm):
        k = k.value

    if not isinstance(k, int) or k > 0:
        beg = [0] * ndim
        end = data.shape[:-1] + [k if isinstance(k, int) else gsm_data_generator.te.size_var("dim")]
        strides = [1] * ndim
