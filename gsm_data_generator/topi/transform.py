    ...
    def _compute(*idxs):
        ...
        for i in range(0, len(idxs)):
            if i not in real_axis:
                dim = gsm_data_generator.tir.if_then_else(a.shape[len(indices)] != 1, idxs[i], 0)
                indices.append(dim)
                axis_index += 1
        return a(*indices)
...
def strided_slice(a, begin, end, strides=None, axes=None, slice_mode="end", assume_inbound=True):
    ...
    """
    if (
        isinstance(begin, gsm_data_generator.te.Tensor)
        or isinstance(end, gsm_data_generator.te.Tensor)
        or isinstance(strides, gsm_data_generator.te.Tensor)
    ):
        assert axes is None, "axes argument is not supported by dynamic strided slice yet."
        if not isinstance(begin, gsm_data_generator.te.Tensor):
            begin = const_vector(begin)
        if not isinstance(end, gsm_data_generator.te.Tensor):
            end = const_vector(end)
        if strides is None:
            strides = [1] * begin.shape[0].value
        if not isinstance(strides, gsm_data_generator.te.Tensor):
...
@gsm_data_generator.te.tag_scope(tag=tag.INJECTIVE + ",strided_set")
def strided_set(a, v, begin, end, strides=None):
    ...
    def _max(a, b):
        return gsm_data_generator.tir.Select(a > b, a, b)

    if strides is None:
        strides = [gsm_data_generator.tir.const(1, "int32")] * n
    else:
        strides = [
            gsm_data_generator.tir.if_then_else(strides.shape[0] > i, strides[i], gsm_data_generator.tir.const(1, "int32"))
            for i in range(n)
        ]
    ...
    begin = [
        gsm_data_generator.tir.if_then_else(
            begin.shape[0] > i,
            begin[i],
            gsm_data_generator.tir.Select(strides[i] > 0, gsm_data_generator.tir.const(0, "int32"), a.shape[i]),
        )
        for i in range(n)
    ]
    end = [
        gsm_data_generator.tir.if_then_else(
            ...
            gsm_data_generator.tir.Select(strides[i] > 0, a.shape[i] + 1, -(a.shape[i] + 1)),
        )
        for i in range(n)
    ]
    ...
    for i in range(n):
        begin[i] = gsm_data_generator.tir.if_then_else(begin[i] < 0, begin[i] + a.shape[i], begin[i])
        end[i] = gsm_data_generator.tir.if_then_else(end[i] < 0, end[i] + a.shape[i], end[i])
    ...
    def _select(*indices):
        ...
        return gsm_data_generator.tir.if_then_else(gsm_data_generator.tir.all(*from_val), v(*index_tuple), a(*indices))
...
def trilu(data, k, upper):
    ...
    if k.dtype != "int32":
        k = gsm_data_generator.tir.Cast("int32", k)

    # Check either above or below diagonal depending on upper.
    check_op = gsm_data_generator.tir.GE
    if upper:
        check_op = gsm_data_generator.tir.LE
    ...
    def _apply_trilu(*indices):
        ...
        if row_index.dtype != col_index.dtype:
            ...
            if row_index.dtype != target_type:
                row_index = gsm_data_generator.tir.Cast(target_type, row_index)
            else:
                col_index = gsm_data_generator.tir.Cast(target_type, col_index)
        other_indices = indices[:-2]
        check_position = check_op(row_index, col_index - k)
        value = data(*other_indices, row_index, col_index)
        return gsm_data_generator.tir.Select(check_position, value, gsm_data_generator.tir.const(0, data.dtype))
