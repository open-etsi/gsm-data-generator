...
def bitserial_dense(
    ...
    matmul_unipolar = te.compute(
        ...
        lambda i, j: te.sum(
            (
                gsm_data_generator.tir.popcount(weight_packed[j, wb, k] & data_packed[i, db, k])
                - gsm_data_generator.tir.popcount(~weight_packed[j, wb, k] & data_packed[i, db, k])
            ).astype(out_dtype)
    ...
    matmul = te.compute(
        oshape,
        lambda i, j: te.sum(
            gsm_data_generator.tir.popcount(weight_packed[j, wb, k] & data_packed[i, db, k]).astype(out_dtype)
            << (db + wb).astype(out_dtype),
            axis=[wb, db, k],
        ),
        tag="bitserial_dense",
