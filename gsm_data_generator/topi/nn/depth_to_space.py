...
def depth_to_space(data, block_size, layout="NCHW", mode="DCR"):
    ...
    if layout == "NCHW":
        ...
        channel_factor = gsm_data_generator.tir.truncdiv(in_c, (block_size * block_size))
    ...
    def _get_pixel(n, c, y, x):
        block_x = gsm_data_generator.tir.truncdiv(x, block_size)
        block_y = gsm_data_generator.tir.truncdiv(y, block_size)
        idx_x = gsm_data_generator.tir.truncmod(x, block_size)
        idx_y = gsm_data_generator.tir.truncmod(y, block_size)
        if mode == "DCR":
