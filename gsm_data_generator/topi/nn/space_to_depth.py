...
def space_to_depth(data, block_size, layout="NCHW"):
    ...
    if layout == "NCHW":
        ...
        output_shape = [
            ...
            gsm_data_generator.tir.truncdiv(in_h, block_size),
            gsm_data_generator.tir.truncdiv(in_w, block_size),
        ]
    elif layout == "NHWC":
    ...
    def _get_pixel(n, c, y, x):
        block_offset = gsm_data_generator.tir.truncdiv(c, in_c)
        channel_idx = gsm_data_generator.tir.truncmod(c, in_c)
        x_idx = gsm_data_generator.tir.truncmod(block_offset, block_size)
        y_idx = gsm_data_generator.tir.truncdiv(block_offset, block_size)
