...
from gsm_data_generator.topi.transform import concatenate
...
def bitpack(data, bits, pack_axis, bit_axis, pack_type, name="QuantizeInput"):
    ...
    def _bitpack(*indices):
        packed_data = [gsm_data_generator.tir.const(0, pack_type)] * bits
        for k in range(data_width):
            ...
            element = data(*idx)
            for b in range(bits):
                extracted_bit = ((element & gsm_data_generator.tir.const(masks[b], "int32")) >> b).astype(
                    pack_type
                )
