...
def batch_matmul(
    ...
    assert XK == YK or isinstance(YK, gsm_data_generator.tir.expr.Var), "shapes of x and y are inconsistent"
    k = te.reduce_axis((0, XK), name="k")
    if oshape is None:
        assert XB == YB or XB == 1 or YB == 1, "batch dimension doesn't match"
        batch = (
            gsm_data_generator.tir.expr.SizeVar("batch", "int32")
            if isinstance(XB, gsm_data_generator.tir.expr.Var) or isinstance(YB, gsm_data_generator.tir.expr.Var)
            else te.max(XB, YB)
        )
        oshape = (batch, XI, YJ)
