...
def relu(x):
    ...
    y : tvm.te.Tensor
        The result.
    """
    return te.compute(x.shape, lambda *i: gsm_data_generator.te.max(x(*i), gsm_data_generator.tir.const(0, x.dtype)))
...
def leaky_relu(x, alpha):
    ...
    def _compute(*indices):
        value = x(*indices)
        calpha = gsm_data_generator.tir.const(alpha, value.dtype)
        return gsm_data_generator.tir.Select(value > 0, value, value * calpha)
...
def softplus(x, beta=1.0, threshold=20.0):
    ...
    def _compute(*indices):
        value = x(*indices)
        b = gsm_data_generator.tir.const(beta, value.dtype)
        t = gsm_data_generator.tir.const(threshold, value.dtype)

        return gsm_data_generator.tir.Select(
            b * value > t, value, (1 / b) * gsm_data_generator.tir.log(1 + gsm_data_generator.tir.exp(b * value))
        )
...
def prelu(x, slope, axis=1):
    ...
    def _compute_channelwise(*indices):
        xval = x(*indices)
        return gsm_data_generator.tir.Select(xval > 0, xval, xval * slope(indices[axis]))
