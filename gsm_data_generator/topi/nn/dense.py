...
def matmul(
    ...
    if not isinstance(reduce_dim_a, gsm_data_generator.tir.Var) and not isinstance(reduce_dim_b, gsm_data_generator.tir.Var):
    ...
    for idx, (l, r) in enumerate(zip(batch_dims_a, batch_dims_b)):
        if (
            not isinstance(l, gsm_data_generator.tir.Var)
            and not isinstance(r, gsm_data_generator.tir.Var)
            and int(l) != 1
            and int(r) != 1
        ):
            assert int(l) == int(r), (
        ...
        if not isinstance(l, gsm_data_generator.tir.Var) and int(l) == 1:
    ...
    def compute(*indices):
        ...
        batch_indices_a = [
            i if isinstance(dim, gsm_data_generator.tir.Var) or int(dim) != 1 else 0
...
def dense_pack(data, weight, bias=None, out_dtype=None):
    ...
    idxdiv = gsm_data_generator.tir.indexdiv
    idxmod = gsm_data_generator.tir.indexmod
