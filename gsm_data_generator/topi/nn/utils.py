...
def infer_stride(data, kernel, out):
    ...
    _, _, OH, OW = out.shape
    hstride = (IH - KH) // gsm_data_generator.te.max(OH - 1, 1) + gsm_data_generator.tir.Select(OH == 1, 1, 0)
    wstride = (IW - KW) // gsm_data_generator.te.max(OW - 1, 1) + gsm_data_generator.tir.Select(OW == 1, 1, 0)
