...
def pixel_shuffle(data, upscale_factor, name="PixelShuffle"):
    ...
    upscale_factor_const = gsm_data_generator.tir.const(upscale_factor, "int32")
    c_in, h_in, w_in = data.shape[-3], data.shape[-2], data.shape[-1]

    c_out = gsm_data_generator.tir.floordiv(c_in, upscale_factor_const * upscale_factor_const)
    ...
    def _compute(*indices):
        ...
        h_idx = gsm_data_generator.tir.floordiv(h_out_idx, upscale_factor_const)
        h_offset = h_out_idx % upscale_factor_const

        w_idx = gsm_data_generator.tir.floordiv(w_out_idx, upscale_factor_const)
    ...
    return gsm_data_generator.te.compute(out_shape, _compute, name=name)
