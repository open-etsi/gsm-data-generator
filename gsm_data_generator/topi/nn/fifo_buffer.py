...
@gsm_data_generator.te.tag_scope(tag=tag.INJECTIVE + ",fifo_buffer")
def fifo_buffer(data, buffer, axis):
    ...
    * Implementing a FIFO queue to cache intermediate results, e.g. as in Fast WaveNet.
    ...
    if len(buffer.shape) == 1:
        return te.compute(
            buffer.shape,
            lambda i: gsm_data_generator.tir.if_then_else(
    ...
    if len(buffer.shape) == 2:
        if axis == 0:
            return te.compute(
                buffer.shape,
                lambda i, j: gsm_data_generator.tir.if_then_else(
                    i < buflen - data_size,
        ...
        if axis == 0:
            return te.compute(
                buffer.shape,
                lambda i, j, k: gsm_data_generator.tir.if_then_else(
                    i < buflen - data_size,
        ...
        if axis == 0:
            return te.compute(
                buffer.shape,
                lambda i, j, k, l: gsm_data_generator.tir.if_then_else(
                    i < buflen - data_size,
