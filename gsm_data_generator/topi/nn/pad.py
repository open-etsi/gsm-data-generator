...
from gsm_data_generator.tir import if_then_else
...
@gsm_data_generator.te.tag_scope(tag=tag.INJECTIVE + ",pad")
def pad(data, pad_before, pad_after=None, pad_value=0.0, name="PadInput", attrs=None):
    ...
    pad_value = (
        pad_value
        if isinstance(pad_value, gsm_data_generator.tir.PrimExpr)
        else gsm_data_generator.tir.const(pad_value, data.dtype)
    )
    ...
    def _pad(*indices):
        ...
        if not_zero:
            not_zero = gsm_data_generator.tir.all(*not_zero)
            return gsm_data_generator.tir.if_then_else(not_zero, data(*index_tuple), pad_value)
...
def mirror_pad(data, pad_before, pad_after=None, mode="SYMMETRIC", name="MirrorPadInput"):
    ...
    def _pad(*indices):
        ...
        for i, axis in enumerate(index_tuple):
            mapped_axis = gsm_data_generator.tir.if_then_else(below[i], -axis - mode, axis)
            mapped_axis = gsm_data_generator.tir.if_then_else(
    ...
    def _pad(*indices):
        ...
        for i in range(data.ndim):
            ...
            clamped_idx = if_then_else(
                orig_idx < 0,
                gsm_data_generator.tir.const(0, "int32"),  # replicate first element
                if_then_else(
                    orig_idx >= size,
...
def circular_pad(data, pad_before, pad_after=None, name="CircularPadInput"):
    ...
    def _pad(*indices):
        ...
        for i in range(data.ndim):
            ...
            orig_idx = idx - before
            wrapped_idx = gsm_data_generator.tir.indexmod(orig_idx + size, size)
