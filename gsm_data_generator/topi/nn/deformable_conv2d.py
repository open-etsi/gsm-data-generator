...
def deformable_conv2d_nchw(
    ...
    zero = gsm_data_generator.tir.const(0.0, data.dtype)

    def _bilinear(n, c, h, w):
        outside = gsm_data_generator.tir.any(h < 0, w < 0, h >= in_height, w >= in_width)
        val = bilinear_sample_nchw(data, (n, c, h, w), in_height - 1, in_width - 1)
        return gsm_data_generator.tir.if_then_else(outside, zero, val)
