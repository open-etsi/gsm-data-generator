...
@gsm_data_generator.ffi.register_object("ir.BaseFunc")
class BaseFunc(RelaxExpr):
    ...
    def with_attr(self, attr_key_or_dict, attr_value=None) -> "BaseFunc":
        ...
        if isinstance(attr_key_or_dict, dict):
            for key, val in attr_key_or_dict.items():
                res = _ffi_api.BaseFuncWithAttr(res._move(), key, gsm_data_generator.runtime.convert(val))
            return res

        return _ffi_api.BaseFuncWithAttr(
            res._move(), attr_key_or_dict, gsm_data_generator.runtime.convert(attr_value)
        )
    ...
    def with_attrs(self, attr_map: Union[DictAttrs, Dict[str, Object]]) -> "BaseFunc":
        ...
        func : BaseFunc
            A new copy of the function
        """
        if isinstance(attr_map, gsm_data_generator.ir.DictAttrs):
