...
@gsm_data_generator.ffi.register_object("ir.BaseExpr")
class BaseExpr(Node):
...
@gsm_data_generator.ffi.register_object("ir.PrimExpr")
...
@gsm_data_generator.ffi.register_object("ir.RelaxExpr")
class RelaxExpr(BaseExpr):
    """Base class of all non-primitive expressions."""

    @property
    def struct_info(self) -> Optional["gsm_data_generator.relax.StructInfo"]:
...
@gsm_data_generator.ffi.register_object("ir.GlobalVar")
class GlobalVar(RelaxExpr):
    ...
    def __call__(self, *args: RelaxExpr) -> BaseExpr:
        ...
        if all(isinstance(x, RelaxExpr) for x in args):
            ...
            return gsm_data_generator.tir.call_tir(self, *args)

        arg_types = [type(x) for x in args]
        raise RuntimeError(f"Do not know how to handle GlobalVar.__call__ for types {arg_types}")


@gsm_data_generator.ffi.register_object("ir.Range")
