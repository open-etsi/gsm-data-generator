...
from gsm_data_generator.meta_schedule.runner import EvaluatorConfig
...
if TYPE_CHECKING:
    from gsm_data_generator.meta_schedule.runner import RPCConfig
...
def benchmark(
    ...
    *,
    ...
    target = gsm_data_generator.target.Target.current() if target is None else gsm_data_generator.target.Target(target)
    if target is None:
    ...
    input_tensors: List[Union[gsm_data_generator.nd.NDArray, int]] = []
    ...
    for input_shape, input_dtype in input_infos:
        if input_dtype == "scalar":
            ...
            input_tensors.append(
                gsm_data_generator.nd.array(generate_input_data(list(input_shape), input_dtype), device=dev)
            )
    # append scalar input tensors for rotary embedding
    input_tensors.extend(scalar_input_tensors)
    # build locally
    rt_mod = gsm_data_generator.tir.build(mod, target=target)
    ...
    if rpc_config is None:
        ...
        from gsm_data_generator.testing import rpc_run  # pylint: disable=import-outside-toplevel

        _, profile_result = rpc_run(
            rt_mod,
            device_type=dev.DEVICE_TYPE_TO_NAME[dev.device_type],
            args=[w.numpy() if isinstance(w, gsm_data_generator.nd.NDArray) else w for w in input_tensors],
...
def benchmark_relax_func(
    ...
    relax_func: Union[gsm_data_generator.ir.GlobalVar, str],
    sample_number: int = 2,
    dym_var_sample_func: Callable[
        [Dict[str, str]],
        Dict[str, int],
    ] = default_dym_var_sample_func,
    target: Union[str, gsm_data_generator.target.Target] = "llvm -num-cores=4",
    evaluator_config: Optional["EvaluatorConfig"] = None,
    rpc_config: Optional["RPCConfig"] = None,
) -> None:
    ...
    if isinstance(relax_func, str):
        for gv in relax_funcs:  # pylint: disable=invalid-name
            if get_func_name_from_gv(gv) == relax_func:
                relax_func = gv
                break
        if not isinstance(relax_func, gsm_data_generator.ir.GlobalVar):
            raise ValueError(
