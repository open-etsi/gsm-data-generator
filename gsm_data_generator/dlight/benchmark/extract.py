...
def extract_shape(
    ...
    """
    if isinstance(arg, (tuple, list, gsm_data_generator.relax.Tuple)):
        results = []
        for sub_arg in arg:
...
def extract_dynamic_var(
    func_dict: Dict[
        gsm_data_generator.ir.GlobalVar,
        Dict[
            gsm_data_generator.ir.GlobalVar,
            List[Tuple[List, int]],
        ],
    ],
) -> Dict[gsm_data_generator.ir.GlobalVar, Dict[str, str]]:
    ...
    """
    dym_var_dict: Dict[gsm_data_generator.ir.GlobalVar, Dict[str, str]] = {}
                ...
                for arg in flattened_arg_list:
                    if isinstance(arg, relax.TensorStructInfo):
                        for val in arg.shape.values:
                            if isinstance(val, gsm_data_generator.tir.Var):
...
def extract_func_info_from_prim_func(
...
) -> Tuple[List[Tuple[Tuple[Union[gsm_data_generator.tir.Var, int], ...], str]], Dict[str, str]]:
    ...
    for param in func.params:
        ...
        for dim in buffer.shape:
            if isinstance(dim, gsm_data_generator.tir.IntImm):
                shape.append(dim.value)
            elif isinstance(dim, gsm_data_generator.tir.Var):
...
def extract_all_func_info_from_relax(
    mod: gsm_data_generator.ir.IRModule,
) -> Tuple[
    Dict[gsm_data_generator.ir.GlobalVar, Dict[gsm_data_generator.ir.GlobalVar, List[Tuple[List, int]]]],
    Dict[gsm_data_generator.ir.GlobalVar, Dict[str, str]],
]:
    ...
    result : Tuple[
        ...
        The function input information and dynamic shape variable dictionary.
    """
    relax_func_dict: Dict[gsm_data_generator.ir.GlobalVar, Dict[gsm_data_generator.ir.GlobalVar, List[Tuple[List, int]]]] = {}
    for gv, func in mod.functions_items():  # pylint: disable=invalid-name,too-many-nested-blocks
        if isinstance(func, gsm_data_generator.relax.Function):
            for block in func.body.blocks:
                for binding in block.bindings:
                    if isinstance(binding.value, gsm_data_generator.relax.expr.Call):
                        raw_args = binding.value.args
                        functor = raw_args[0]
                        if isinstance(functor, gsm_data_generator.ir.GlobalVar) and isinstance(
                            mod.functions[functor], gsm_data_generator.tir.PrimFunc
                        ):
                            args = extract_shape(raw_args[1:]) + extract_shape(binding.value)
                            if isinstance(functor, gsm_data_generator.ir.GlobalVar):
...
def extract_prim_func(  # pylint: disable=too-many-arguments
    ...
    func: gsm_data_generator.tir.PrimFunc,
    *,
    func_args: Optional[List[Tuple[Tuple[Union[gsm_data_generator.relax.expr.Call, int], ...], str]]] = None,
    dym_var_dict: Optional[Dict[str, str]] = None,
    weight: int = 1,
    sample_number: int = 5,
    target: Optional[Union[str, gsm_data_generator.target.Target]] = None,
) -> str:
    ...
    elif isinstance(target, gsm_data_generator.target.Target):
        target_str = str(target)
    else:
    ...
    return SKETCH.format(
        **{
            ...
            "relax_func_name": relax_func_name,
            "prim_func_name": prim_func_name,
            "func_hash": gsm_data_generator.ir.structural_hash(func),
            "weight": weight,
            "sample_number": sample_number,
