...
def get_func_name_from_gv(gv: gsm_data_generator.ir.GlobalVar) -> str:  # pylint: disable=invalid-name
...
def dym_var_sample_str(sample: Dict[Union[str, gsm_data_generator.relax.expr.Call], int]) -> str:
        ...
        if isinstance(input_info, relax.struct_info.ShapeStructInfo):
            ...
            for dim in tensor_shape:
                if isinstance(dim, int):
                    shape.append(dim)
                elif isinstance(dim, gsm_data_generator.tir.IntImm):
