...
from gsm_data_generator.runtime.ndarray import NDArray
...
class Trainer:
    ...
    def zero_init_params(self):
        ...
        self._params = [
            gsm_data_generator.nd.array(np.zeros(self._get_shape_list(p), p.struct_info.dtype), self.device)
            for p in self._param_vars
        ]
    ...
    def zero_init_states(self):
        ...
        self._states = [
            gsm_data_generator.nd.array(np.zeros(self._get_shape_list(s), s.struct_info.dtype), self.device)
    ...
    def load_params(
        ...
        if isinstance(params, list):
            ...
            self._params = [gsm_data_generator.nd.array(v, self.device) for v in params]
        elif isinstance(params, dict):
            for key, val in params.items():
                if key not in self._param_name_to_pos:
                    raise ValueError(f"Parameter {key} is not found in the model")
                self._params[self._param_name_to_pos[key]] = gsm_data_generator.nd.array(val, self.device)
        else:
    ...
    def load_states(
        ...
        if isinstance(states, list):
            ...
            self._states = [gsm_data_generator.nd.array(v, self.device) for v in states]
        elif isinstance(states, dict):
            for key, val in states.items():
                if key not in self._param_name_to_pos:
                    raise ValueError(f"Parameter {key} is not found in the model")
                self._states[self._param_name_to_pos[key]] = gsm_data_generator.nd.array(val, self.device)
        else:
    ...
    def predict(self, *input_instances: Union[np.ndarray, NDArray]) -> NDArray:
        ...
        *input_instances : Union[np.ndarray, NDArray]
        ...
        all_inputs: List[NDArray] = (
            [gsm_data_generator.nd.array(i, self.device) for i in input_instances] + self._params + self._states
        )
    ...
    def update(
        ...
        all_inputs: List[NDArray] = (
            [gsm_data_generator.nd.array(i, self.device) for i in input_instances]
            + self._params
            + self._states
            + [gsm_data_generator.nd.array(i, self.device) for i in targets]
        )
    ...
    def profile_adjoint(
    ...
    ) -> gsm_data_generator.runtime.profiling.Report:
        ...
        all_inputs: List[NDArray] = (
            [gsm_data_generator.nd.array(i) for i in input_instances]
            + self._params
            + self._states
            + [gsm_data_generator.nd.array(i) for i in targets]
        )
