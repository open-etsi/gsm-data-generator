...
class Optimizer:
    ...
    name: str
    param_list: List[Var]
    state: gsm_data_generator.ir.Array
    ...
    def _set_params_and_dtype(self, params: List[Var]) -> None:
        ...
        for x in params:
            ...
            data_type = gsm_data_generator.DataType(x.struct_info.dtype)
            if not data_type.type_code in (gsm_data_generator.DataTypeCode.BFLOAT, gsm_data_generator.DataTypeCode.FLOAT):
                raise ValueError(
...
class SGD(Optimizer):
    ...
    def init(self, params: Union[Var, List[Var]]) -> "SGD":
        ...
        self.state = (
            # num_steps = 0
            gsm_data_generator.nd.array(np.zeros((), "int64")),
        )
        return self
...
class MomentumSGD(Optimizer):
    ...
    def init(self, params: Union[Var, List[Var]]) -> "MomentumSGD":
        ...
        self.state = (
            ...
            *(
                gsm_data_generator.nd.array(np.zeros(_get_shape_as_int_list(p), p.struct_info.dtype))
...
class Adam(Optimizer):
    ...
    def init(self, params: Union[Var, List[Var]]) -> "Adam":
        ...
        self.state = (
            ...
            gsm_data_generator.nd.array(np.zeros((), "int64")),
            gsm_data_generator.nd.array(np.ones((), self.dtype)),
            gsm_data_generator.nd.array(np.ones((), self.dtype)),
            # first_momentum
            *(
                gsm_data_generator.nd.array(np.zeros(_get_shape_as_int_list(p), p.struct_info.dtype))
