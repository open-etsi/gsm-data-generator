...
def detach_params(mod: gsm_data_generator.IRModule) -> Tuple[gsm_data_generator.IRModule, Dict[str, List[gsm_data_generator.nd.NDArray]]]:
    ...
    """
    detached_mod = gsm_data_generator.IRModule()
    params_dict = dict()
    for gv, func in mod.functions_items():
        if "params" in func.attrs:
            params = list(func.attrs["params"])
            if not all([isinstance(param, gsm_data_generator.nd.NDArray) for param in params]):
                raise ValueError(
