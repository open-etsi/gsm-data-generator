...
from gsm_data_generator.relax import build as relax_build
...
def relax_dynamo(pipeline: Optional[gsm_data_generator.transform.Pass] = None):
    ...
    def _relax_backend(graph_module, example_inputs):
        ...
        def to_torch_tensor(nd_tensor):
            ...
            if isinstance(nd_tensor, gsm_data_generator.nd.NDArray):
                return torch.from_numpy(nd_tensor.numpy())
            elif isinstance(nd_tensor, gsm_data_generator.ir.Array):
        ...
        def to_tvm_tensor(torch_tensor):
            ...
            if not isinstance(torch_tensor, torch._subclasses.fake_tensor.FakeTensor):
                return gsm_data_generator.nd.array(torch_tensor.numpy())
            # Fake Tensor
            real_tensor = torch.randn(torch_tensor.shape, dtype=torch_tensor.dtype)
            return gsm_data_generator.nd.array(real_tensor.numpy())
            ...
            for s in tensor.shape:
                if isinstance(s, torch.SymInt):
                    if str(s) not in shape_vars:
                        shape_vars[str(s)] = gsm_data_generator.tir.Var(str(s), "int64")
        ...
        if device.type == "cuda":
            dev = gsm_data_generator.cuda(device.index)
            target = gsm_data_generator.target.cuda()
        else:
            dev = gsm_data_generator.cpu(0)
            target = gsm_data_generator.target.Target(llvm_target())

        # invoke optimization pipeline.
        if pipeline is None:
            # get default pipeline
            seq = gsm_data_generator.relax.get_pipeline()
        elif isinstance(pipeline, str):
            # lookup by name
            seq = gsm_data_generator.relax.get_pipeline(pipeline)
        else:
            seq = pipeline
        ...
        vm = gsm_data_generator.relax.VirtualMachine(ex.mod, device=dev)
...
def dynamo_capture_subgraphs(model, *params, **kwargs) -> gsm_data_generator.IRModule:
