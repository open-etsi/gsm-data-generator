...
from gsm_data_generator.runtime import Module, load_static_library
    ...
    def __getitem__(self, func_name: str) -> Callable:
        ...
        def _call(*input_args):
            def _convert(arg, name: str):
                from gsm_data_generator import relax as rx  # pylint: disable=import-outside-toplevel
    ...
    **Caveats.** It is required to call `nn.add_extern` to register external modules exactly once
    ...
    def tvm_home() -> Path:
        ...
        if os.environ.get("TVM_HOME", None):
            ...
            import gsm_data_generator  # pylint: disable=import-outside-toplevel

            tvm_path = Path(gsm_data_generator.__file__).parent
            assert tvm_path.is_dir()
