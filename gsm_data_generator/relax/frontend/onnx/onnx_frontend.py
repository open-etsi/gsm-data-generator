...
from gsm_data_generator import TVMError, relax, tir, topi
from gsm_data_generator.ir import IRModule
from gsm_data_generator.ir.supply import NameSupply
from gsm_data_generator.tir.generic import cast
...
def get_value(token, value_dict: Dict[str, gsm_data_generator.tir.SizeVar]) -> Union[int, gsm_data_generator.tir.SizeVar]:
    ...
    try:
        ...
        if token not in value_dict or token == "?":
            value_dict[token] = gsm_data_generator.tir.SizeVar(token, "int64")
        value = value_dict[token]
        return value


def parse_shape_name(
    name: str, value_dict: Dict[str, gsm_data_generator.tir.SizeVar]
) -> Union[tir.PrimExpr, gsm_data_generator.tir.SizeVar]:
...
def get_info(
    info_proto: onnx.onnx_ml_pb2.ValueInfoProto, value_dict: Dict[str, gsm_data_generator.tir.SizeVar]
...
class Equal(OnnxOpConverter):
    ...
    def _impl_v13(cls, bb, inputs, attr, params):
        if all([isinstance(inp, relax.Constant) for inp in inputs]):
            ...
            output = [gsm_data_generator.ir.structural_equal(l, r) for l, r in zip(lhs, rhs)]
...
class Hardmax(OnnxOpConverter):
    ...
    def _impl_v13(cls, bb, inputs, attr, params):
        ...
        on_value = relax.PrimValue(gsm_data_generator.tir.const(1.0, dtype))
        off_value = relax.PrimValue(gsm_data_generator.tir.const(0.0, dtype))
    ...
    def _impl_v11(cls, bb, inputs, attr, params):
        ...
        if "auto_pad" in attr:
            ...
            if attr["auto_pad"] in ("SAME_UPPER", "SAME_LOWER"):
                ...
                raise gsm_data_generator.error.OpAttributeInvalid(msg)
            attr.pop("auto_pad")
...
class Pad(OnnxOpConverter):
    ...
    def _impl_v2(cls, bb, inputs, attr, params):
        ...
        if not pad_mode in ["constant", "edge", "reflect"]:
            raise gsm_data_generator.error.OpAttributeInvalid(
    ...
    def _impl_v13(cls, bb, inputs, attr, params):
        ...
        if isinstance(shape, relax.ShapeExpr):
            ...
            for i, s in enumerate(target_shape):
                if isinstance(s, gsm_data_generator.tir.IntImm) and (
                    (isinstance(data_shape[i], gsm_data_generator.tir.IntImm) and s < data_shape[i])
                    or s.value == -1
                ):
        ...
        for i in range(shape_ndim):
            shape_vars.append(gsm_data_generator.tir.Var("x_%d" % i, "int64"))
...
class Flatten(OnnxOpConverter):
    ...
    def _impl_v13(cls, bb, inputs, attr, params):
        ...
        if axis == 0:
            ...
            shape_flags = [isinstance(x, gsm_data_generator.script.tir.IntImm) for x in data_shape[0:axis]]
...
class ArgMax(OnnxOpConverter):
    ...
    def _impl_v12(cls, bb, inputs, attr, params):
        ...
        if select_last_index:
            ...
            raise gsm_data_generator.error.OpAttributeUnImplemented(
...
class ONNXGraphImporter:
    ...
    def _check_for_unsupported_ops(self, graph: onnx.onnx_ml_pb2.GraphProto):
        ...
        if unsupported_ops:
            ...
            raise gsm_data_generator.error.OpNotImplemented(msg)
    ...
    def _parse_array(self, tensor_proto: onnx.onnx_ml_pb2.TensorProto) -> gsm_data_generator.nd.array:
        np_array = get_numpy(tensor_proto).reshape(tuple(tensor_proto.dims))
        return gsm_data_generator.nd.array(np_array)
