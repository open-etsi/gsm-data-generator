...
def zero_pipeline(*, enable_warning: bool = False):
    ...
    def f_zero_pipeline(mod: gsm_data_generator.ir.IRModule, ctx: gsm_data_generator.transform.PassContext) -> gsm_data_generator.ir.IRModule:
...
def default_build_pipeline():
    ...
    @gsm_data_generator.transform.module_pass(opt_level=0)
    def _pipeline(mod: gsm_data_generator.ir.IRModule, _ctx: gsm_data_generator.transform.PassContext) -> gsm_data_generator.ir.IRModule:
        seq = gsm_data_generator.transform.Sequential(
            [
                ...
                [
                    ...
                    *pre_tuning_layout_rewrite,
                    ...
                    (
                        ...
                        if total_trials > 0
                        else gsm_data_generator.transform.Sequential([])
                    ),
...
def get_pipeline(name: str = "zero", **kwargs) -> gsm_data_generator.transform.Pass:
...
def library_dispatch_passes(target: gsm_data_generator.target.Target):
...
def legalize_passes(target: gsm_data_generator.target.Target):
...
def dataflow_lower_passes(target: gsm_data_generator.target.Target):
    """Get the default legalization passes for the given target."""
    if target.kind.name == "cuda":
...
def finalize_passes(target: gsm_data_generator.target.Target):
    """Get the default legalization passes for the given target."""
    if target.kind.name == "cuda":
