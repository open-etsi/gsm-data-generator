...
from gsm_data_generator.arith import Analyzer
...
def _check_matmul(context: PatternCheckContext) -> bool:
    ...
    if lhs_dtype == "int8" and rhs_dtype == "int8":
        if lhs_shape[-1] % 4 != 0:
            ...
            return False
        if not isinstance(rhs_shape[-1], (gsm_data_generator.tir.expr.IntImm, int)) or rhs_shape[-1] % 4 != 0:
        ...
        if matmul_rhs_var in context.matched_bindings:
            ...
            assert (
                isinstance(matmul_rhs_call, gsm_data_generator.relax.Call)
        ...
        if (
            not isinstance(rhs_shape[-1], (gsm_data_generator.tir.expr.IntImm, int))
            or rhs_shape[-1] % (16 // DataType(lhs_dtype).itemsize) != 0
        ):
            return False
        if (
            not isinstance(rhs_shape[-2], (gsm_data_generator.tir.expr.IntImm, int))
    ...
    if "bias" in context.annotated_expr:
        ...
        if not isinstance(bias_batches, (gsm_data_generator.tir.expr.IntImm, int)) or int(bias_batches) > 1:
    ...
    return (
        isinstance(lhs_batches, gsm_data_generator.tir.Var)
        or isinstance(rhs_batches, gsm_data_generator.tir.Var)
