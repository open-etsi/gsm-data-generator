...
def _compile_flashinfer_kernels(
    ...
    compute_version = "".join(gsm_data_generator.contrib.nvcc.get_target_compute_version(target).split("."))
...
def _load_flashinfer_modules(object_files: List[Path]) -> List[gsm_data_generator.runtime.Module]:
    return [
        gsm_data_generator.runtime.load_static_library(str(obj_path.absolute()), func_names=[])
...
def gen_flashinfer_prefill_module(
    ...
    num_threads: int = 8,
) -> List[gsm_data_generator.runtime.Module]:
