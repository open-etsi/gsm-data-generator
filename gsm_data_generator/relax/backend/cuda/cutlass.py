...
from gsm_data_generator.relax.dpl import rewrite_call
from gsm_data_generator.relax.dpl.pattern import GlobalVarPattern, TuplePattern, is_op, wildcard
...
def _check_conv2d(context: PatternCheckContext) -> bool:
    ...
    for dim in data.struct_info.shape.values:
        if isinstance(dim, gsm_data_generator.tir.Var):
            return False
...
class WorkspaceAnnotator(PyExprMutator):
    ...
    def visit_function_(self, f):
        ...
        if "attention" in f.attrs["Composite"] and "cutlass" in f.attrs["Composite"]:
            ...
            if not isinstance(workspace_size_bytes, (int, gsm_data_generator.tir.expr.IntImm)):
