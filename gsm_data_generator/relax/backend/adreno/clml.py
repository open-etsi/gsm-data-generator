...
from gsm_data_generator import relax, IRModule
from gsm_data_generator.ir.transform import PassContext, module_pass
from gsm_data_generator.relax import transform
from gsm_data_generator.relax.expr_functor import PyExprMutator, mutator
from gsm_data_generator.relax.expr import TupleGetItem, VarBinding
...
class AppendReshapeToBNRewriter(PyExprMutator):
    ...
    def visit_tuple_getitem_(self, op: TupleGetItem):
        ...
        reshape_op = gsm_data_generator.ir.Op.get("relax.reshape")
...
class AppendReshapeToBNRewriterPass:
    def transform_function(
        self, func: relax.Function, mod: IRModule, _ctx: gsm_data_generator.transform.PassContext
    ) -> relax.Function:
...
def clml_sdk_version():
    ...
    return int(gsm_data_generator.support.libinfo().get("TVM_CLML_VERSION", 2))
...
def is_clml_runtime_enabled():
    ...
    """
    check_enabled = gsm_data_generator.get_global_func("relax.op.is_openclml_runtime_enabled", True)
    if check_enabled:
