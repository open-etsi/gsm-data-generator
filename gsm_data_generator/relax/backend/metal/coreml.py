...
from gsm_data_generator.contrib import coreml_runtime
from gsm_data_generator.contrib.xcode import compile_coreml

import gsm_data_generator
from gsm_data_generator.relax import transform
from gsm_data_generator.relax.struct_info import TensorStructInfo, PrimStructInfo
...
class CodegenCoreML(PyExprVisitor):
    ...
    def __init__(self, model_name, function):
        ...
        getter = gsm_data_generator.get_global_func("relax.analysis.get_var2val")
...
@gsm_data_generator.ffi.register_func("relax.ext.coreml")
def coreml_compiler(funcs, options, constant_names):
    ...
    compiled_funcs = []
    for func in funcs:
        assert isinstance(func, gsm_data_generator.relax.Function)
