...
from gsm_data_generator import relax, topi, tir
from gsm_data_generator.relax.op.grad.grad import end_checkpoint, start_checkpoint
...
class Module(gsm_data_generator.relax.frontend.nn.SubroutineMixin):
...
def init_params(mod: gsm_data_generator.IRModule) -> List[gsm_data_generator.nd.array]:
    ...
    for k, v in shape_dict.items():
        ...
        if isinstance(v, relax.ShapeExpr):
            ...
            params.append(gsm_data_generator.nd.array(np.zeros(shape).astype(np.float32)))
        else:
            raise TypeError("cannot initialize for unknown-shape parameters.")
    return params
