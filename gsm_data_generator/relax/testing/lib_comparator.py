...
class LibCompareVMInstrument:
    ...
    def compare(
        self,
        name: str,
        ref_args: Union[List[gsm_data_generator.nd.NDArray], Tuple[gsm_data_generator.nd.NDArray, ...]],
        new_args: Union[List[gsm_data_generator.nd.NDArray], Tuple[gsm_data_generator.nd.NDArray, ...]],
    ...
    def __call__(self, func, name, before_run, ret_val, *args):
        ...
        if name.startswith("vm.builtin."):
            return
        if any(not isinstance(x, gsm_data_generator.nd.NDArray) for x in args):
            return
        try:
        ...
        for i, arg in enumerate(args):
            arr = gsm_data_generator.nd.empty(arg.shape, arg.dtype, device=self.device)
            # copy from cpu since we look at different device
            if i not in ret_indices:
                temp_cpu = arg.copyto(gsm_data_generator.cpu())
