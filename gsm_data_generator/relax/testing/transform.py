...
from gsm_data_generator.relax.expr import Call, DataflowBlock, Var
from gsm_data_generator.runtime.object import Object


def ApplyEmptyCppMutator() -> gsm_data_generator.ir.transform.Pass:
    """Create empty cpp mutator"""
    packed_func = gsm_data_generator.get_global_func("relax.testing.transform.ApplyEmptyCppMutator")
    return packed_func()
...
def dataflow_liveness_analysis(block: DataflowBlock) -> Dict[Var, Tuple[int, int]]:
    ...
    live_ranges = gsm_data_generator.get_global_func("relax.testing.transform.DataflowLivenessAnalysis")(
        block
    )  # type: ignore
    ret = {}
...
def dataflow_alias_analysis(
    ...
    alias_sets, tuple_map = gsm_data_generator.get_global_func("relax.testing.transform.DataflowAliasAnalysis")(
        block,
        inputs,
    )  # type: ignore
    res_alias_sets = {}
...
@gsm_data_generator.ffi.register_object("relax.transform.InplaceOpportunity")
...
def dataflow_inplace_analysis(
    ...
    index_lists = gsm_data_generator.get_global_func("relax.testing.transform.DataflowInplaceAnalysis")(
        block, inputs, mod
    )  # type: ignore
...
def dataflow_single_inplace_call(
    ...
    ret = gsm_data_generator.get_global_func("relax.testing.transform.SingleInplaceCall")(
        mod,
        call,
        inplace_indices,
    )  # type: ignore
