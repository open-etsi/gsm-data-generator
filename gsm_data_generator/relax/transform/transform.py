...
from gsm_data_generator.ir.container import Array
from gsm_data_generator.relax import Expr, Var, StructInfo
from gsm_data_generator.relax.dpl import DFPattern
from gsm_data_generator.runtime import NDArray, Object
from gsm_data_generator.tir import IndexMap, PrimFunc
...
@gsm_data_generator.ffi.register_object("relax.FunctionPass")
class FunctionPass(gsm_data_generator.ir.transform.Pass):
    """A pass that works on each tvm.relax.Function in a module. A function
    pass class should be created through `function_pass`.
    """


@gsm_data_generator.ffi.register_object("relax.DataflowBlockPass")
class DataflowBlockPass(gsm_data_generator.ir.transform.Pass):
...
) -> gsm_data_generator.ir.transform.Pass:
...
def ToNonDataflow() -> gsm_data_generator.ir.transform.Pass:
...
def TopologicalSort(order="depth-first", direction="from-inputs") -> gsm_data_generator.ir.transform.Pass:
...
def RemovePurityChecking() -> gsm_data_generator.ir.transform.Pass:
...
def DataflowUseInplaceCalls() -> gsm_data_generator.ir.transform.Pass:
    """
...
def LambdaLift() -> gsm_data_generator.ir.transform.Pass:
...
def LazyGetInput() -> gsm_data_generator.ir.transform.Pass:
...
def LazySetOutput() -> gsm_data_generator.ir.transform.Pass:
...
def ConvertToDataflow(min_size: int = 2) -> gsm_data_generator.ir.transform.Pass:
...
def CallTIRRewrite() -> gsm_data_generator.ir.transform.Pass:
...
def Normalize() -> gsm_data_generator.ir.transform.Pass:
...
def NormalizeGlobalVar() -> gsm_data_generator.ir.transform.Pass:
...
def CanonicalizeBindings() -> gsm_data_generator.ir.transform.Pass:
    """
...
def UpdateVDevice(new_vdevice: gsm_data_generator.ir.VDevice, index: int) -> gsm_data_generator.ir.transform.Pass:
...
def RewriteDataflowReshape() -> gsm_data_generator.ir.transform.Pass:
...
def StaticPlanBlockMemory() -> gsm_data_generator.ir.transform.Pass:
...
def LowerAllocTensor() -> gsm_data_generator.ir.transform.Pass:
...
def KillAfterLastUse() -> gsm_data_generator.ir.transform.Pass:
...
def ComputePrimValue() -> gsm_data_generator.ir.transform.Pass:
...
def LowerRuntimeBuiltin() -> gsm_data_generator.ir.transform.Pass:
...
def VMBuiltinLower() -> gsm_data_generator.ir.transform.Pass:
    """Lowering generic intrinsic to VM intrinsics.
...
def VMShapeLower(*, emit_err_ctx: bool = True) -> gsm_data_generator.ir.transform.Pass:
...
def AttachGlobalSymbol() -> gsm_data_generator.ir.transform.Pass:
...
def BindParams(
    func_name: str,
    params: Dict[Union[str, Var], Union[gsm_data_generator.runtime.NDArray, np.ndarray]],
) -> gsm_data_generator.ir.transform.Pass:
    ...
    for k, v in params.items():
        if isinstance(v, np.ndarray):
            v = gsm_data_generator.nd.array(v)
        assert isinstance(v, (gsm_data_generator.runtime.NDArray, gsm_data_generator.relax.Constant)), (
...
def BindSymbolicVars(
    binding_map: Mapping[Union[str, gsm_data_generator.tir.Var], gsm_data_generator.tir.PrimExpr],
...
def RunCodegen(
    ...
    from gsm_data_generator.contrib import cutlass as _cutlass

    return _ffi_api.RunCodegen(target_options, entry_functions)  # type: ignore


def FoldConstant() -> gsm_data_generator.ir.transform.Pass:
...
def ExpandTupleArguments() -> gsm_data_generator.ir.transform.Pass:
...
def RemoveUnusedParameters() -> gsm_data_generator.ir.transform.Pass:
...
def RemoveUnusedOutputs() -> gsm_data_generator.ir.transform.Pass:
...
def InlinePrivateFunctions() -> gsm_data_generator.ir.transform.Pass:
...
def AnnotateTIROpPattern() -> gsm_data_generator.ir.transform.Pass:
...
def FuseOps(fuse_opt_level=-1) -> gsm_data_generator.ir.transform.Pass:
...
def FuseTIR() -> gsm_data_generator.ir.transform.Pass:
...
@gsm_data_generator.ffi.register_object("relax.transform.PatternCheckContext")
...
@gsm_data_generator.ffi.register_object("relax.transform.FusionPattern")
...
def MergeCompositeFunctions() -> gsm_data_generator.ir.transform.Pass:
...
def AttachAttrLayoutFreeBuffers() -> gsm_data_generator.ir.transform.Pass:
...
def SplitLayoutRewritePreproc() -> gsm_data_generator.ir.transform.Pass:
...
def LiftTransformParams(shared_transform: Union[bool, List[str]] = False) -> gsm_data_generator.ir.transform.Pass:
...
def BundleModelParams(param_tuple_name: Optional[str] = None) -> gsm_data_generator.ir.transform.Pass:
...
def RealizeVDevice() -> gsm_data_generator.ir.transform.Pass:
...
def DecomposeOpsForInference(func_name: Optional[str] = None) -> gsm_data_generator.ir.transform.Pass:
...
def DecomposeOpsForTraining(func_name: Optional[str] = None) -> gsm_data_generator.ir.transform.Pass:
...
def ConvertLayout(desired_layouts: Dict[str, List[str]]) -> gsm_data_generator.ir.transform.Pass:
...
def DeadCodeElimination(entry_functions: Optional[List[str]] = None) -> gsm_data_generator.ir.transform.Pass:
...
def SplitCallTIRByPattern(patterns: List[PrimFunc], fcodegen: Callable) -> gsm_data_generator.ir.transform.Pass:
...
def RewriteCUDAGraph() -> gsm_data_generator.ir.transform.Pass:
...
def AllocateWorkspace() -> gsm_data_generator.ir.transform.Pass:
...
def function_pass(
    ...
    def create_function_pass(pass_arg):
        ...
        info = gsm_data_generator.transform.PassInfo(opt_level, fname, required, traceable)
