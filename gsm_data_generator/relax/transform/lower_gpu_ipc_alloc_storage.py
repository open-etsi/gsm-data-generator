...
from gsm_data_generator.relax.expr import Expr
from gsm_data_generator.relax.expr_functor import PyExprMutator, mutator


@gsm_data_generator.transform.module_pass(opt_level=0, name="LowerGPUIPCAllocStorage")
class LowerGPUIPCAllocStorage:
    """Lower the storage/tensor allocation on IPC memory."""

    def transform_module(self, mod: IRModule, _ctx: gsm_data_generator.transform.PassContext) -> IRModule:
...
class _Rewriter(PyExprMutator):
    def __init__(self, mod: IRModule) -> None:
        super().__init__(mod)
        self.mod = mod
        self.memory_alloc_storage_op = gsm_data_generator.ir.Op.get("relax.memory.alloc_storage")
        self.memory_alloc_tensor_op = gsm_data_generator.ir.Op.get("relax.memory.alloc_tensor")
        self.builtin_alloc_tensor_op = gsm_data_generator.ir.Op.get("relax.builtin.alloc_tensor")
