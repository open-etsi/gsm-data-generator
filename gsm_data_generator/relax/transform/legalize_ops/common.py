...
from gsm_data_generator.tir import FloatImm, IntImm
...
def _try_convert_to_scalar_const(
    ...
    if isinstance(expr, Constant) and expr.struct_info.ndim == 0:
        ...
        if dtype.startswith("float"):
            return gsm_data_generator.tir.FloatImm(dtype, value)
        elif dtype.startswith("int") or dtype.startswith("uint") or dtype.startswith("bool"):
            return gsm_data_generator.tir.IntImm(dtype, value)
    return expr
...
def register_legalize(op_name: str, legal_func: LegalizeFunc = None):
    ...
    """
    return gsm_data_generator.ir.register_op_attr(op_name, _LEGALIZE_ATTR_NAME, legal_func)
