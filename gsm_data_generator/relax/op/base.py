...
from gsm_data_generator.runtime.object import Object
from gsm_data_generator.runtime import ObjectGeneric
...
def register_gradient(
    ...
    level: int
        The priority level
    """
    return gsm_data_generator.ir.register_op_attr(op_name, "FPrimalGradient", fgradient, level)
...
def _wrap_inline_arg_tuple(args) -> Expr:
    ...
    if (
        ...
        and not isinstance(args, gsm_data_generator.relax.Tuple)
        and (
            args.struct_info_ is None
            or not isinstance(args.struct_info_, gsm_data_generator.relax.TupleStructInfo)
        )
    ):
        return gsm_data_generator.relax.Tuple([args])
    else:
        return args
...
def render_object(val: gsm_data_generator.Object) -> str:
    """
    ...
    """
    if isinstance(val, gsm_data_generator.nd.NDArray):
        return str(val)
    if isinstance(val, gsm_data_generator.ir.Array):
        fields = ", ".join([render_object(val[i]) for i in range(len(val))])
        return f"({fields})"
    return str(val)


@gsm_data_generator.register_func("relax.run.shape_to_tensor")
def relax_shape_to_tensor(shape_tuple: gsm_data_generator.runtime.ShapeTuple) -> gsm_data_generator.nd.NDArray:
    """
    ...
    """
    return gsm_data_generator.nd.array([int(v) for v in shape_tuple])


@gsm_data_generator.register_func("relax.run.print")
def relax_print(format_str: str, *format_args: gsm_data_generator.Object) -> None:
    """
...
@gsm_data_generator.register_func("relax.run.assert_op")
def relax_assert_op(condition: gsm_data_generator.Object, format_str: str, *format_args: gsm_data_generator.Object) -> None:
    """
    ...
    if isinstance(condition, (bool, int)):
        val = condition
    elif isinstance(condition, gsm_data_generator.nd.NDArray):
...
def assert_op(
    ...
    if not isinstance(condition, Expr):
        condition = gsm_data_generator.relax.PrimValue(condition)
