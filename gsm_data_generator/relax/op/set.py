...
@gsm_data_generator.register_func("relax.run.unique")
def numpy_unique(
    x: gsm_data_generator.nd.array,
    sorted: int,
    return_index: int,
    return_inverse: int,
    return_counts: int,
    ...
    if sorted:
        return gsm_data_generator.nd.array(output_sorted_numpy)
    output_numpy = np.take(x_numpy, builtins.sorted(indices), axis=axis)
    return gsm_data_generator.nd.array(output_numpy)
...
@gsm_data_generator.register_func("relax.run.nonzero")
def numpy_nonzero(x: gsm_data_generator.nd.array) -> gsm_data_generator.nd.array:
    np_result = np.atleast_1d(x.numpy()).nonzero()
    return gsm_data_generator.nd.array(np.stack(np_result, axis=0))
