...
from gsm_data_generator.libinfo import find_lib_path
from gsm_data_generator.runtime.module import load_module as _load_module
from gsm_data_generator.contrib import utils
from gsm_data_generator.contrib.popen_pool import PopenWorker
...
def _server_env(load_library, work_path=None):
    ...
    @gsm_data_generator.ffi.register_func("tvm.rpc.server.workpath", override=True)
    def get_workpath(path):
        return temp.relpath(path)

    @gsm_data_generator.ffi.register_func("tvm.rpc.server.load_module", override=True)
    ...
    @gsm_data_generator.ffi.register_func("tvm.rpc.server.download_linked_module", override=True)
    def download_linked_module(file_name):
        ...
        if path.endswith(".o"):
            ...
            from gsm_data_generator.contrib import cc as _cc

            _cc.create_shared(path + ".so", path)
            path += ".so"
        elif path.endswith(".tar"):
            # Extra dependencies during runtime.
            from gsm_data_generator.contrib import cc as _cc, tar as _tar
