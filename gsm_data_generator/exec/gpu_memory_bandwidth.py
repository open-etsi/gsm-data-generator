...
from gsm_data_generator.testing import local_run, rpc_run
...
def main():  # pylint: disable=too-many-locals
    ...
    args = _parse_args()
    # pylint: disable=invalid-name
    target = gsm_data_generator.target.Target(args.target)
    if args.target_host is not None:
        target = gsm_data_generator.target.Target(args.target, host=args.target_host)
    dtype = args.dtype
    rpcConfig = None
    ...
    for len_bx, len_tx, len_vec in itertools.product(
        ...
        if rpcConfig is None:
            _, profile_result = local_run(
                gsm_data_generator.compile(sch.mod, target=target),
                target.kind.name,
                [a, b],
