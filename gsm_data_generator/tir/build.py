...
def split_host_device_mods(mod: IRModule) -> Tuple[IRModule, Dict[Target, IRModule]]:
    ...
    def is_host_func(f):
        target = f.attrs.get("target", gsm_data_generator.target.Target("llvm"))
        return str(target.kind) in ["llvm", "c"]

    host_mod = gsm_data_generator.tir.transform.Filter(is_host_func)(mod)
    device_mod = gsm_data_generator.tir.transform.Filter(lambda f: not is_host_func(f))(mod)
    ...
    for target_str in target_str2target.keys():
        ...
        device_mod_dict[target] = gsm_data_generator.IRModule(device_func_dict[target_str], attrs=device_mod.attrs)
    return host_mod, device_mod_dict


def codegen_build(mod: IRModule, target: Target) -> gsm_data_generator.runtime.Module:
    """Build a runtime module from an IRModule and a Target."""
    if gsm_data_generator.ir.transform.PassContext.current().config.get("tir.disable_assert", False):
        mod = gsm_data_generator.tir.transform.SkipAssert()(mod)
    build_f_name = "target.build." + target.kind.name
    bf = gsm_data_generator.get_global_func(build_f_name)
    if bf is None:
...
def build(
    ...
    pipeline: Union[None, str, gsm_data_generator.transform.Pass] = "default",
):
    ...
    target_host = "llvm" if gsm_data_generator.runtime.enabled("llvm") else "c"
    if target is not None:
    ...
    mod = gsm_data_generator.tir.transform.BindTarget(target_to_bind)(mod)

    # Step 4: Apply the tir pipeline
    if pipeline is not None:
        # custom pipeline
        if isinstance(pipeline, str):
            pipeline = gsm_data_generator.tir.get_tir_pipeline(pipeline)
    else:
        # default pipeline depends on the target
        pipeline = gsm_data_generator.tir.get_default_tir_pipeline(target)
    mod = pipeline(mod)
    ...
    host_mod = gsm_data_generator.tir.pipeline.finalize_host_passes()(host_mod)
    device_mod_dict = {
        target: gsm_data_generator.tir.pipeline.finalize_device_passes()(device_mod)
        for target, device_mod in device_mod_dict.items()
    }
...
gsm_data_generator.register_func("tir.build", build)
