...
from gsm_data_generator.ir import Array, Op, PrimExpr
...
def call_llvm_intrin(dtype, name, *args, span=None):
    ...
    return call_intrin(
        ...
        gsm_data_generator.tir.const(llvm_id, "uint32"),
        *args,
        span=span,
    )
...
def call_tir(global_var: gsm_data_generator.ir.GlobalVar, *args):
    ...
    call : PrimExpr
        The call expression.
    """
    assert isinstance(global_var, gsm_data_generator.ir.GlobalVar)
...
@gsm_data_generator.ffi.register_func("tvm.default_trace_action")
...
def trace(args, trace_action="tvm.default_trace_action"):
    ...
    return gsm_data_generator.tir.Call(args[-1].dtype, Op.get("tir.tvm_call_trace_packed"), call_args)
...
def comm_reducer(fcombine, fidentity, name="reduce"):
    ...
    def _make_reduce(expr, axis, where=None, init=None):
        ...
        if isinstance(expr, Array):
            ...
            assert isinstance(expr, gsm_data_generator.ir.PrimExpr)
            size = 1
            dtype = expr.dtype
        ...
        if not isinstance(axis, (list, tuple, gsm_data_generator.ir.Array)):
            axis = [axis]
        if where is None:
            where = tir.convert(True)
        if init is None:
            outputs = tuple(gsm_data_generator.tir.Reduce(combiner, expr, axis, where, i, []) for i in range(size))
        else:
            outputs = tuple(
                gsm_data_generator.tir.Reduce(combiner, expr, axis, where, i, init) for i in range(size)
            )
        return outputs[0] if size == 1 else outputs

    # pylint: disable=keyword-arg-before-vararg
    def reducer(expr, axis, where=None, init=None, *args):
        if isinstance(axis, (gsm_data_generator.tir.IterVar, list, tuple)):
            assert not args
...
def get_vscale_expr(dtype: Union[str, gsm_data_generator.ffi.dtype], min_size: int = 128) -> PrimExpr:
    """
    ...
    if isinstance(dtype, str):
        dtype = gsm_data_generator.ffi.dtype(dtype)
