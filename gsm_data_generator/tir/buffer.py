...
from gsm_data_generator.ir import PointerType, PrimExpr, PrimType, Range
from gsm_data_generator.runtime import Object, Scriptable, convert

from . import _ffi_api


@gsm_data_generator.ffi.register_object("tir.Buffer")
        ...
        if has_slice and not has_step:
            ...
            for i, index in enumerate(indices):
                if isinstance(index, slice):
                    ...
                    if isinstance(start, gsm_data_generator.tir.expr.PrimExpr) and isinstance(step, int):
                        step = gsm_data_generator.tir.expr.IntImm(start.dtype, step)
...
@gsm_data_generator.ffi.register_object("tir.DataProducer")
