...
from gsm_data_generator.ir import BaseFunc, Range
from gsm_data_generator.runtime import Object, Scriptable
...
@gsm_data_generator.ffi.register_object("tir.PrimFunc")
class PrimFunc(BaseFunc, Scriptable):
    ...
    def __init__(
        ...
        for x in params:
            x = gsm_data_generator.runtime.convert(x) if not isinstance(x, Object) else x
        ...
        if attrs is None:
            attrs = gsm_data_generator.ir.make_node("ir.DictAttrs")
...
@gsm_data_generator.ffi.register_object("tir.TensorIntrin")
...
@gsm_data_generator.ffi.register_object("tir.IndexMap")
        ...
        *,
        ...
        for name, param in params.items():
            if param.kind in [
                ...
                args.append(gsm_data_generator.tir.Var(name, index_dtype))

            elif param.kind == inspect.Parameter.VAR_POSITIONAL:
                var_arg_name = name

            elif param.kind == inspect.Parameter.KEYWORD_ONLY:
                kwargs[name] = gsm_data_generator.tir.Var(name, index_dtype)
        ...
        if var_arg_name is not None:
            ...
            for i in range(num_var_args):
                args.append(gsm_data_generator.tir.Var(f"{var_arg_name}_{i}", index_dtype))
        ...
        if is_iterable:
            for val in mapping:
                if isinstance(val, gsm_data_generator.ir.PrimExpr):
