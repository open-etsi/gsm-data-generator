...
def exists():
    ...
    """
    func = gsm_data_generator.get_global_func("tvm.contrib.cudnn.exists", allow_missing=True)
    if func is None:
        return False
...
def conv_output_shape(
    ...
    if tensor_format == 0:
        ...
        x_chan = x_shape[1]
        w_chan_input = w_shape[1]
        w_lanes = gsm_data_generator.runtime.DataType(conv_dtype).lanes
        assert w_lanes == 1
        x_shape = x_shape[2:]
        w_shape = w_shape[2:]
    ...
    x_lanes = gsm_data_generator.runtime.DataType(data_dtype).lanes
...
def _conv_find_algo(
    ...
    func = gsm_data_generator.ffi.get_global_func(func_name)
    return func(
        tensor_format,
        dims - 2,
...
def conv_forward(
    ...
    if isinstance(x.shape[0], gsm_data_generator.tir.expr.IntImm):
...
def conv_backward_data(
    ...
    assert isinstance(
        dy.shape[0], gsm_data_generator.tir.expr.IntImm
...
def conv_backward_filter(
    ...
    assert isinstance(
        x.shape[0], gsm_data_generator.tir.expr.IntImm
