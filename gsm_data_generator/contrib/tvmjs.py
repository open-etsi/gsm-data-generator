...
def dump_ndarray_cache(
    params: Union[
        Mapping[str, Union[np.ndarray, gsm_data_generator.runtime.NDArray]],
        Iterator[Tuple[str, Union[np.ndarray, gsm_data_generator.runtime.NDArray]]],
    ],
    cache_dir: str,
    ...
    for k, origin_v in param_generator:
        ...
        dtype = origin_v.dtype if isinstance(origin_v, gsm_data_generator.nd.NDArray) else v.dtype
...
def load_ndarray_cache(cachepath: str, device: gsm_data_generator.runtime.Device):
    ...
    for shard_rec in json_info["records"]:
        ...
        for rec in shard_rec["records"]:
            ...
            arr = gsm_data_generator.nd.empty(shape, dtype, device=device)
