...
from gsm_data_generator.ffi.registry import register_func
from gsm_data_generator.contrib.nvcc import get_cuda_version
...
def has_cutlass():
    ...
    return gsm_data_generator.get_global_func("relax.ext.cutlass", True) is not None
...
def select_gemm_kernel(
    ...
    workloads."""
    if any(isinstance(s, gsm_data_generator.tir.Any) for s in [MM, KK, NN]):
...
def handle_conv2d(
    ...
    if any(isinstance(s, gsm_data_generator.tir.Any) for s in d_shape):
        out = cutlass_profiler.get_default(
...
def _extract_arg_idx(pattern_name, f):
    extract_func = gsm_data_generator.get_global_func("relax.contrib.extract_arg_idx")
    arg_indices = extract_func(pattern_name, f)
    return {k: int(v) for k, v in arg_indices.items()}


def is_shape_valid_for_cutlass_matmul(
    lhs_shape: Sequence[gsm_data_generator.ir.PrimExpr],
    rhs_shape: Sequence[gsm_data_generator.ir.PrimExpr],
) -> bool:
    """
    ...
    """
    if not isinstance(lhs_shape[-1], (gsm_data_generator.tir.expr.IntImm, int)):
...
def profile_relax_function(functions, options):
    ...
    for f in functions:
        annotator = CutlassRelaxFunctionAnnotator(
            gsm_data_generator.IRModule.from_expr(f), conv2d_profiler, gemm_profiler, options
        )
...
def compile_cutlass_module(c_source_module, options):
    ...
    return gsm_data_generator.runtime.load_static_library(lib_path, function_names)
