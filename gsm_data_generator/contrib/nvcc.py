...
def compile_cuda(code, target_format=None, arch=None, options=None, path_target=None):
    ...
    use_nvshmem = (
        gsm_data_generator.get_global_func("runtime.nvshmem.cumodule_init", allow_missing=True) is not None
    )
    if use_nvshmem:
        target_format = "cubin"
    ...
    pass_context = gsm_data_generator.get_global_func("transform.GetCurrentPassContext")()
    kernels_output_dir = (
...
@gsm_data_generator.ffi.register_func
...
@gsm_data_generator.ffi.register_func("tvm_callback_libdevice_path")
...
@gsm_data_generator.ffi.register_func("tvm.contrib.nvcc.get_compute_version")
def get_target_compute_version(target=None):
    ...
    if gsm_data_generator.cuda(0).exist:
        return gsm_data_generator.cuda(0).compute_version
...
def have_tensorcore(compute_version=None, target=None):
    ...
    if compute_version is None:
        if gsm_data_generator.cuda(0).exist:
            compute_version = gsm_data_generator.cuda(0).compute_version
        else:
...
@gsm_data_generator.ffi.register_func("tvm.contrib.nvcc.supports_bf16")
...
@gsm_data_generator.ffi.register_func("tvm.contrib.nvcc.supports_fp8")
...
@gsm_data_generator.ffi.register_func("tvm.contrib.nvcc.supports_fp4")
