...
class MSCArray(object):
    ...
    def _analysis(self, data: Any) -> Tuple[str, str, np.ndarray]:
        ...
        if isinstance(data, gsm_data_generator.runtime.NDArray):
            device = gsm_data_generator.runtime.Device.DEVICE_TYPE_TO_NAME[data.device.device_type]
            if data.device.device_id:
                device += ":{}".format(data.device.device_id)
            return MSCFramework.TVM, "tensor", device
        if isinstance(data, gsm_data_generator.relax.Var):
    ...
    def _to_device(self, device: str) -> Any:
        ...
        if self._framework == MSCFramework.TVM:
            return gsm_data_generator.nd.array(self._cast_data(), device=self.get_device(device))
        return self._meta_data
    ...
    def cast(self, framework: str, device: str = "cpu") -> Any:
        ...
        if framework == MSCFramework.TVM:
            return gsm_data_generator.nd.array(data, device=self.get_device(device, framework))
        return data
    ...
    def get_device(self, device: str, framework: str = None) -> Any:
        ...
        if framework == MSCFramework.TVM:
            ...
            if device.startswith("cuda"):
                ...
                return gsm_data_generator.cuda(dev_id)
    ...
    def is_array(cls, data: Any) -> bool:
        ...
        normal_types = (np.ndarray, gsm_data_generator.runtime.NDArray, gsm_data_generator.relax.Var)
...
def get_version(framework: str) -> List[int]:
    ...
    try:
        if framework in (MSCFramework.MSC, MSCFramework.TVM):
            raw_version = gsm_data_generator.__version__
        ...
        if framework == MSCFramework.TENSORRT:
            raw_version = ".".join(
                [str(v) for v in gsm_data_generator.get_global_func("relax.get_tensorrt_version")()]
            )
        else:
            raw_version = "1.0.0"
    except:  # pylint: disable=bare-except
