...
from gsm_data_generator.contrib.msc.core.ir import MSCGraph
from gsm_data_generator.contrib.msc.core.frontend import from_relax
from gsm_data_generator.contrib.msc.core.codegen import to_relax
from gsm_data_generator.contrib.msc.core.tools import BaseTool, ToolType, ToolScope, create_tool, remove_tools
from gsm_data_generator.contrib.msc.core.utils.namespace import MSCFramework
from gsm_data_generator.contrib.msc.core.utils.message import MSCStage
from gsm_data_generator.contrib.msc.core import utils as msc_utils
from gsm_data_generator.contrib.msc.core import _ffi_api
...
class BaseRunner(object):
    ...
    def build(
        ...
        if not self._graphs and cache_info.get("graphs"):
            ...
            with open(cache_dir.relpath(cache_info["weights"]), "rb") as f:
                self._weights = gsm_data_generator.runtime.load_param_dict(f.read())
            build_msg += "Load "
    ...
    def save_cache(
        ...
        with cache_dir:
            with open(cache_info["weights"], "wb") as f_params:
                f_params.write(gsm_data_generator.runtime.save_param_dict(self._weights))
    ...
    def translate(self, apply_hooks: bool = True) -> Tuple[List[MSCGraph], Dict[str, gsm_data_generator.nd.array]]:
    ...
    def _translate(self, mod: gsm_data_generator.IRModule) -> Tuple[List[MSCGraph], Dict[str, gsm_data_generator.nd.array]]:
        """Translate IRModule to MSCgraphs
    ...
    def reset_tools(
        ...
        weights: List[Dict[str, gsm_data_generator.nd.array]] = None,
    ...
    def _generate_model(self, graphs: List[MSCGraph], weights: Dict[str, gsm_data_generator.nd.array]) -> Any:
        """Codegen the model according to framework
    ...
    def export_module(self, folder: msc_utils.MSCDirectory) -> gsm_data_generator.IRModule:
    ...
    def get_weights(self, framework: str = None, device: str = None) -> Iterable[gsm_data_generator.nd.array]:
    ...
    def get_runtime_params(self) -> Dict[str, gsm_data_generator.nd.array]:
    ...
    def _get_runtime_params(self) -> Dict[str, gsm_data_generator.nd.array]:
        """Get the runtime parameters
...
class BYOCRunner(BaseRunner):
    ...
    def _load_graphs(self, cache_dir: msc_utils.MSCDirectory, cache_info: dict) -> List[MSCGraph]:
        ...
        with open(cache_dir.relpath(cache_info["byoc_mod"]), "r") as f:
            self._byoc_mod = gsm_data_generator.ir.load_json(f.read())
    ...
    def _save_graphs(self, cache_dir: msc_utils.MSCDirectory) -> dict:
        ...
        with cache_dir:
            ...
            with open("byoc_module.json", "w") as f:
                f.write(gsm_data_generator.ir.save_json(self._byoc_mod))
        return {
    ...
    def _build_runnable(self, model: Any) -> Any:
        ...
        model = gsm_data_generator.relax.transform.LegalizeOps()(model)
        if self._device == "cpu":
            target = gsm_data_generator.target.Target("llvm")
            with gsm_data_generator.transform.PassContext(opt_level=3):
                self._executable = gsm_data_generator.compile(model, target)
                runnable = gsm_data_generator.relax.VirtualMachine(self._executable, gsm_data_generator.cpu())
        elif self._device.startswith("cuda"):
            target = gsm_data_generator.target.Target("cuda")
            with target:
                model = gsm_data_generator.tir.transform.DefaultGPUSchedule()(model)
            with gsm_data_generator.transform.PassContext(opt_level=3):
                self._executable = gsm_data_generator.compile(model, target)
                runnable = gsm_data_generator.relax.VirtualMachine(self._executable, gsm_data_generator.cuda())
        else:
            raise NotImplementedError("Unsupported device " + str(self._device))
        return runnable

    def _call_runnable(
        self, runnable: gsm_data_generator.relax.VirtualMachine, inputs: Dict[str, np.ndarray], device: str
    ) -> Union[List[np.ndarray], Dict[str, np.ndarray]]:
        """Call the runnable to get outputs
    ...
    def support_device(cls, device: str) -> bool:
        ...
        if device.startswith("cuda"):
            ...
            return gsm_data_generator.cuda(dev_id).exist
        return False
