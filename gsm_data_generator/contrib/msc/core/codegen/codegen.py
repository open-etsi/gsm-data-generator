...
def to_relax(
    graph: MSCGraph,
    weights: Optional[Dict[str, gsm_data_generator.nd.array]] = None,
    codegen_config: Optional[Dict[str, str]] = None,
    print_config: Optional[Dict[str, str]] = None,
    build_folder: msc_utils.MSCDirectory = None,
    plugin: Any = None,
    ...
    def _to_var(tensor: MSCTensor):
        ...
        dims = [
            d if isinstance(d, int) else gsm_data_generator.tir.Var(d, "int64") for d in tensor.get_shape(True)
        ]
        return gsm_data_generator.relax.Var(v_name, gsm_data_generator.relax.TensorStructInfo(dims, tensor.dtype_name))

    def _save_weights(folder: msc_utils.MSCDirectory):
        if weights:
            with open(folder.relpath(graph.name + "_params.bin"), "wb") as f_params:
                f_params.write(gsm_data_generator.runtime.save_param_dict(weights))

    # pylint: disable=unused-argument
    def _post_proc(mod: gsm_data_generator.IRModule, folder: msc_utils.MSCDirectory) -> gsm_data_generator.IRModule:
        ...
        passes.extend(
            [
                ...
                gsm_data_generator.relax.transform.CanonicalizeBindings(),
                gsm_data_generator.relax.transform.ConvertToDataflow(min_size=1),
            ]
        )
        return gsm_data_generator.ir.transform.Sequential(
            passes, name="tvm.contrib.msc.core.codegen.to_relax_postproc"
        )(mod)

    source_getter = gsm_data_generator.get_global_func("msc.framework.tvm.GetRelaxSources")
