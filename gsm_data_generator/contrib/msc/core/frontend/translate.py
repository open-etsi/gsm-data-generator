...
from gsm_data_generator.relax.backend.pattern_registry import get_patterns_with_prefix
...
from gsm_data_generator.contrib.msc.core.ir import MSCGraph, MSCTensor
        ...
        def _normalize(info):
            ...
            for dim in info:
                if isinstance(dim, int):
                    ...
                    recorded_vars[dim] = gsm_data_generator.tir.Var(dim, "int64")
                    dims.append(recorded_vars[dim])
                else:
...
def normalize_weights(
    t_weights: Dict[MSCTensor, gsm_data_generator.nd.array], graph: MSCGraph
) -> Dict[str, gsm_data_generator.nd.array]:
    ...
    def _to_data(ref_t, data):
        ...
        if weight_t.ndim == 1:
            if ref_t.ndim != weight_t.ndim:
                return gsm_data_generator.nd.array(data.numpy().reshape(weight_t.get_shape()))
            return data
        if ref_t.layout and weight_t.layout:
            ...
            if ref_layout != weight_layout:
                ...
                return gsm_data_generator.nd.array(data.numpy().transpose(*permute))
        return data
...
def from_relax(
    ...
    params: Optional[Dict[str, gsm_data_generator.nd.array]] = None,
    trans_config: Optional[Dict[str, str]] = None,
    build_config: Optional[Dict[str, str]] = None,
    opt_config: Optional[Dict[str, str]] = None,
) -> Tuple[MSCGraph, Dict[str, gsm_data_generator.nd.array]]:
    ...
    passes = [
        gsm_data_generator.relax.transform.ExpandTupleArguments(),
        msc_transform.SetExprName(),
        msc_transform.SetExprLayout(trans_config.get("allow_layout_missing", True)),
        gsm_data_generator.relax.transform.FuseOpsByPattern(
            patterns, bind_constants=False, annotate_codegen=False
        ),
    ]
    mod = gsm_data_generator.transform.Sequential(passes)(mod)
...
class BYOCChecker(PyExprVisitor):
    ...
    def visit_var_binding_(self, binding) -> None:
        ...
        if isinstance(binding.value, gsm_data_generator.relax.Call):
            if isinstance(binding.value.op, gsm_data_generator.relax.GlobalVar):
                if binding.value.op.name_hint not in self._func_names:
                    self._non_target_exprs.append(binding.value)
            else:
                self._non_target_exprs.append(binding.value)
        elif not isinstance(binding.value, gsm_data_generator.relax.DataflowVar):
            self._non_target_exprs.append(binding.value)
...
def byoc_partition(
    ...
    trans_config: Optional[Dict[str, str]] = None,
    build_config: Optional[Dict[str, str]] = None,
) -> Tuple[gsm_data_generator.IRModule, List[Tuple[MSCGraph, Dict[str, gsm_data_generator.nd.array]]]]:
    ...
    def _partition_mod(mod, as_msc=True):
        ...
        passes = [
            ...
            msc_transform.SetExprName(),
            msc_transform.SetExprLayout(trans_config.get("allow_layout_missing", True)),
            gsm_data_generator.relax.transform.FuseOpsByPattern(patterns, bind_constants=not as_msc),
            msc_transform.InlineParams(),
            msc_transform.FuseTuple(target),
            gsm_data_generator.relax.transform.MergeCompositeFunctions(),
            msc_transform.SetBYOCAttrs(target),
        ]
        return gsm_data_generator.transform.Sequential(passes)(mod)
