...
from gsm_data_generator.contrib.msc.core.tools.tool import ToolType, BaseTool, ToolStrategy
...
class BaseDistiller(BaseTool):
    ...
    def _reset(
    ...
    ) -> Tuple[List[MSCGraph], Dict[str, gsm_data_generator.nd.array]]:
        """Reset the tool
        ...
        if self._distilled:
            with open(self._weights_path, "rb") as f:
                distilled_weights = gsm_data_generator.runtime.load_param_dict(f.read())
    ...
    def _save_weights(self, weights: Dict[str, Any]):
        ...
        weights = {n: gsm_data_generator.nd.array(msc_utils.cast_array(d)) for n, d in weights.items()}
