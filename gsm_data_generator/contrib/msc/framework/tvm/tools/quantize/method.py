...
from gsm_data_generator.relax import op as relax_op
from gsm_data_generator.contrib.msc.core.tools.quantize import QuantizeMethod, BaseQuantizer
...
class TVMQuantizeMethod(QuantizeMethod):
    ...
    def get_quantize_cache(
        ...
        data: gsm_data_generator.relax.Var,
        name: str,
        consumer: str,
        scale: float,
        axis: int = -1,
        epsilon: float = 1.0 / (1 << 24),
    ) -> Tuple[gsm_data_generator.relax.Constant, gsm_data_generator.relax.Constant]:
        ...
        if scale_tensor is None:
            ...
            scale_tensor = gsm_data_generator.relax.Constant(gsm_data_generator.nd.array(scale_tensor), span=scale_span)
            zp_span = _ffi_api.SpanCreateWithAttr("name", name_prefix + "_zero_point")
            zero_point = gsm_data_generator.relax.Constant(gsm_data_generator.nd.array(zero_point), span=zp_span)
