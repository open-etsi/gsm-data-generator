...
from gsm_data_generator.contrib.msc.core.tools.tool import ToolType, ToolStrategy
from gsm_data_generator.contrib.msc.core.tools.quantize import BaseQuantizer
...
class TVMQuantizerFactory(object):
    ...
    def create(self, base_cls: BaseQuantizer) -> BaseQuantizer:
        ...
        class Quantizer(base_cls):
            ...
            def _execute_before_build(self, block_builder: gsm_data_generator.relax.BlockBuilder):
            ...
            def _execute_after_build(
                self, output: Union[gsm_data_generator.relax.Var, List[gsm_data_generator.relax.DataflowVar]]
            ) -> List[gsm_data_generator.relax.Var]:
                ...
                if isinstance(output, gsm_data_generator.relax.Var):
                    return super()._execute_after_build([output] + gather_tensors)
                return super()._execute_after_build(output + gather_tensors)

            def _execute_after_forward(
                self, outputs: List[gsm_data_generator.runtime.NDArray]
            ) -> Union[gsm_data_generator.runtime.NDArray, List[gsm_data_generator.runtime.NDArray]]:
            ...
            def _process_tensor(
                self,
                tensor: gsm_data_generator.relax.DataflowVar,
                name: str,
                consumer: str,
                scope: str,
                strategys: List[ToolStrategy],
            ) -> gsm_data_generator.relax.DataflowVar:
                """Process tensor
