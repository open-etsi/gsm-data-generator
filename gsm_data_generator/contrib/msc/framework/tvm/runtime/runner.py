...
from gsm_data_generator.contrib.msc.core.runtime import ModelRunner
from gsm_data_generator.contrib.msc.core.tools import execute_step
...
from gsm_data_generator.contrib.msc.framework.tvm.codegen import to_relax
from gsm_data_generator.contrib.msc.framework.tvm import tools
...
class WrapRunnable(object):
    ...
    def __init__(self, runnable: gsm_data_generator.relax.VirtualMachine, entry: str = "main"):
        self._runnable = runnable
        self._entry = entry

    def __call__(self, *inputs) -> List[gsm_data_generator.nd.array]:
...
class TVMRunner(ModelRunner):
    ...
    def _build_runnable(self, model: Any) -> Any:
        ...
        if self._training:
            model = gsm_data_generator.relax.transform.DecomposeOpsForTraining()(model)
        else:
            model = gsm_data_generator.relax.transform.DecomposeOpsForInference()(model)
    ...
    @classmethod
    def load_native(cls, model: Any, config: dict) -> Tuple[gsm_data_generator.IRModule, str, bool]:
        ...
        if isinstance(model, str) and os.path.isfile(model):
            with open(model, "r") as f:
                native_model = gsm_data_generator.ir.load_json(f.read())
        elif isinstance(model, gsm_data_generator.IRModule):
            native_model = model
        else:
    ...
    def run_native(
        cls,
        model: gsm_data_generator.IRModule,
        ...
        if gsm_data_generator.cuda().exist:
            ...
            with target:
                model = gsm_data_generator.tir.transform.DefaultGPUSchedule()(model)
            with gsm_data_generator.transform.PassContext(opt_level=3):
                relax_exec = gsm_data_generator.compile(model, target)
                runnable = gsm_data_generator.relax.VirtualMachine(relax_exec, gsm_data_generator.cuda())
            tvm_inputs = [gsm_data_generator.nd.array(inputs[i], device=gsm_data_generator.cuda()) for i in input_names]
        else:
            target = gsm_data_generator.target.Target("llvm")
            with gsm_data_generator.transform.PassContext(opt_level=3):
                relax_exec = gsm_data_generator.compile(model, target)
                runnable = gsm_data_generator.relax.VirtualMachine(relax_exec, gsm_data_generator.cpu())
            tvm_inputs = [gsm_data_generator.nd.array(inputs[i]) for i in input_names]
        ...
        if repeat > 0:
            ...
            outputs = _run_once()
            avg_time = -1
        if isinstance(outputs, gsm_data_generator.runtime.NDArray):
            outputs = [outputs]
