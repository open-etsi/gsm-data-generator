...
from gsm_data_generator.contrib.msc.core.utils import MSCFramework
from gsm_data_generator.contrib.msc.core import utils as msc_utils
from gsm_data_generator.contrib.msc.framework.tensorrt import _ffi_api
...
def to_sub_tensorrt(
    graph: MSCGraph,
    weights: Dict[str, gsm_data_generator.nd.array],
...
def to_tensorrt(
    ...
    mod = gsm_data_generator.transform.Sequential(
        [
            gsm_data_generator.relax.transform.RunCodegen({"msc_tensorrt": target_options}),
        ]
    )(mod)
    return mod
