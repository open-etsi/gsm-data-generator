...
from gsm_data_generator.relax.dpl import pattern
from gsm_data_generator.relax.transform import PatternCheckContext, FusionPattern
from gsm_data_generator.relax.backend.pattern_registry import register_patterns
from gsm_data_generator.contrib.msc.core.transform import pattern as msc_pattern
...
def _check_expr(expr: relax.Expr, dtypes: Tuple[str] = None) -> bool:
    ...
    def _check(sinfo):
        ...
        for s in sinfo.shape.values:
            if isinstance(s, gsm_data_generator.tir.IntImm) and s < 0:
                unknown_dim += 1
...
def plugin_attrs_getter(
    annotated_expr: Dict[str, gsm_data_generator.relax.Expr],
) -> Dict[str, str]:
