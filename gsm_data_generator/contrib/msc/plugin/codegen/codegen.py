...
from gsm_data_generator.contrib.msc.plugin import _ffi_api
...
class TVMPluginCodegen(BasePluginCodeGen):
    ...
    def setup(self):
        ...
        super().setup()
        tvm_root = os.path.dirname(os.path.dirname(gsm_data_generator.__path__[0]))
        self._codegen_config.update(
...
class TensorRTPluginCodegen(BasePluginCodeGen):
    ...
    def setup(self):
        """Set up the codegen"""
        # pylint: disable=import-outside-toplevel
        from gsm_data_generator.contrib.msc.framework.tensorrt import _ffi_api as _trt_api
