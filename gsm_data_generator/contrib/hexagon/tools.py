...
import gsm_data_generator.contrib.cc as cc
...
def link_shared(so_name, objs, extra_args=None):
    ...
    def to_str(s):
        if isinstance(s, gsm_data_generator.tir.StringImm):
            return s.value
    ...
    linker = gsm_data_generator.get_global_func("tvm.contrib.hexagon.hexagon_link")()
...
def pack_imports(
    module: gsm_data_generator.runtime.Module,
    ...
    fpack_to_bin = gsm_data_generator.get_global_func(pack_to_bin_f_name)
    assert fpack_to_bin, f"Expecting {pack_to_bin_f_name} in registry"

    fpack_to_bin(module).numpy().tofile(path_bin)

    mblob_symbol = c_symbol_prefix + gsm_data_generator.get_global_func("runtime.ModuleImportsBlobName")()
...
def allocate_hexagon_array(
    ...
    arr = gsm_data_generator.nd.empty(physical_shape, dtype=dtype, device=dev, mem_scope=mem_scope)
