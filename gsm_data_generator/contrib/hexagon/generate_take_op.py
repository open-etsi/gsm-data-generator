...
def op_replace(call_node, func) -> bool:
    if not isinstance(call_node, relax.Call):
        return False
    call_tir_op = gsm_data_generator.ir.Op.get("relax.call_tir")
...
class Tanh2TakeReplace(gsm_data_generator.relax.PyExprMutator):
    def __init__(self, mod: gsm_data_generator.IRModule) -> None:
        super().__init__(mod)
        self.mod_ = mod

    def transform(self) -> gsm_data_generator.IRModule:
        ...
        if call_node.args[1][0].struct_info.dtype == "uint8":
            if op_replace(call_node, func):
                ...
                take_node = relax.call_tir(
                    take_func_gv,
                    relax.expr.Tuple(
                        [call_node.args[1][0], relax.expr.Constant(gsm_data_generator.nd.array(LUT))]
                    ),
...
@gsm_data_generator.ir.transform.module_pass(opt_level=2, name="replace_tanh_take")
