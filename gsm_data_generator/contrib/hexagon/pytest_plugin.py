...
import gsm_data_generator.rpc.tracker
from gsm_data_generator.contrib.hexagon.build import HexagonLauncher, HexagonLauncherRPC
...
requires_hexagon_toolchain = gsm_data_generator.testing.requires_hexagon(support_required="compile-only")
...
def get_free_port() -> int:
    ...
    if gsm_data_generator.testing.utils.IS_IN_CI and not RNG_SEEDED:
        random.seed(0)
        RNG_SEEDED = True
    ...
    while gsm_data_generator.contrib.hexagon.build._is_port_in_use(port):
...
def _tracker_info() -> Union[str, int]:
    ...
    if env_tracker_host or env_tracker_port:
        ...
        try:
            gsm_data_generator.rpc.connect_tracker(env_tracker_host, env_tracker_port)
        ...
        tracker = gsm_data_generator.rpc.tracker.Tracker("127.0.0.1", get_free_port())
        try:
...
aot_host_target = gsm_data_generator.testing.parameter(HEXAGON_AOT_LLVM_TARGET)


@gsm_data_generator.testing.fixture
def aot_target(aot_host_target):
    if aot_host_target == "c":
        yield gsm_data_generator.target.hexagon("v68")
