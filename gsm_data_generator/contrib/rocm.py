...
import gsm_data_generator.target
...
def find_lld(required=True):
    ...
    """
    lld_list = []
    major = gsm_data_generator.target.codegen.llvm_version_major(allow_none=True)
...
@gsm_data_generator.ffi.register_func("tvm_callback_rocm_link")
...
@gsm_data_generator.ffi.register_func("tvm_callback_rocm_bitcode_path")
def callback_rocm_bitcode_path(rocdl_dir=None):
    ...
    return gsm_data_generator.runtime.convert(bitcode_files)
...
def have_matrixcore(compute_version=None):
    ...
    if compute_version is None:
        if gsm_data_generator.rocm(0).exist:
            compute_version = gsm_data_generator.rocm(0).compute_version
        else:
...
@gsm_data_generator.ffi.register_func("tvm_callback_rocm_get_arch")
