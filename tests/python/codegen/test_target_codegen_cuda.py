...
from gsm_data_generator.contrib.nvcc import have_bf16, have_fp16, have_int8
...
def test_cuda_vectorize_add():
    ...
    def check_cuda(dtype, n, lanes):
        if dtype == "float16" and not have_fp16(gsm_data_generator.cuda(0).compute_version):
            print("Skip because gpu does not have fp16 support")
            return
        if dtype == "int8" and not have_int8(gsm_data_generator.cuda(0).compute_version):
        ...
        fun = gsm_data_generator.compile(sch.mod, target="cuda")
...
def test_cuda_bf16_vectorize_add():
    if not have_bf16(gsm_data_generator.cuda(0).compute_version):
    ...
    def check_cuda(n, lanes):
        ...
        a = gsm_data_generator.nd.empty((n,), A.dtype, dev).copyfrom(np_float2np_bf16(np_a))
        c = gsm_data_generator.nd.empty((n,), B.dtype, dev)
        fun(a, c)
        c = gsm_data_generator.nd.empty((n, lanes), "uint16", dev).copyfrom(c)
        gsm_data_generator.testing.assert_allclose(c.numpy(), np_float2np_bf16(np_a + 1))
...
def test_cuda_multiply_add():
    ...
    def check_cuda(dtype, n, lanes):
        ...
        D = te.compute(
            (n,), lambda i: gsm_data_generator.tir.call_pure_extern("int32", "__dp4a", A[i], B[i], C[i]), name="D"
        )
        sch = gsm_data_generator.tir.Schedule(te.create_prim_func([A, B, C, D]))
        ...
        dev = gsm_data_generator.cuda(0)
        a = gsm_data_generator.nd.empty((n,), A.dtype, dev).copyfrom(np_a)
        b = gsm_data_generator.nd.empty((n,), B.dtype, dev).copyfrom(np_b)
        c = gsm_data_generator.nd.empty((n,), C.dtype, dev).copyfrom(np_c)
        d = gsm_data_generator.nd.empty((n,), D.dtype, dev)
        fun(a, b, c, d)
        gsm_data_generator.testing.assert_allclose(d.numpy(), np_d)
...
def test_cuda_vectorize_load():
    ...
    def check_cuda(dtype, n, lanes):
        ...
        np_a = np.random.randint(low=-128, high=127, size=(n, lanes))
        a = gsm_data_generator.nd.empty((n,), A.dtype, dev).copyfrom(np_a)
        b = gsm_data_generator.nd.empty((n,), B.dtype, dev)
        fun(a, b)
        gsm_data_generator.testing.assert_allclose(a.numpy(), b.numpy())
...
def test_cuda_make_int8():
    def check_cuda(n, value, lanes):
        dtype = "int8"
        dev = gsm_data_generator.cuda(0)
        A = te.compute((n, lanes), lambda i, j: gsm_data_generator.tir.const(value, dtype=dtype), name="A")

        sch = gsm_data_generator.tir.Schedule(te.create_prim_func([A]))
        y, x = sch.get_loops("A")
        sch.vectorize(x)
        ...
        a = gsm_data_generator.nd.empty(np_a.shape, dtype, dev)
        fun(a)
...
@gsm_data_generator.testing.parametrize_targets("cuda", "rocm")
def test_crossthread_reduction1(target, dev):
    ...
    def verify(nthd):
        ...
        for kk in [x for x in vals]:
            size = (nn, kk)
            a = gsm_data_generator.nd.array(np.random.uniform(size=size).astype(A.dtype), dev)
            b = gsm_data_generator.nd.array(np.zeros(nn, dtype=B.dtype), dev)
            func(a, b)
            gsm_data_generator.testing.assert_allclose(b.numpy(), np.sum(a.numpy(), axis=1), rtol=1e-3)
...
def test_crossthread_reduction2(target, dev):
    ...
    def sched(nthdx, nthdy):
        ...
        sch.bind(x, "blockIdx.x")
        func = gsm_data_generator.compile(sch.mod, target="cuda")
        return func
    ...
    def verify(nthdx, nthdy):
        ...
        for kk0, kk1 in [(x, y) for x in vx for y in vy]:
            ...
            b = gsm_data_generator.nd.array(np.zeros(nn, dtype=B.dtype), dev)
            func(a, b)
            gsm_data_generator.testing.assert_allclose(b.numpy(), np.sum(a.numpy(), axis=(1, 2)), rtol=1e-3)
...
def test_cuda_const_float_to_half():
    ...
    b = gsm_data_generator.tir.const(0.5, dtype="float16")
    ...
    c = gsm_data_generator.nd.array(c_np, dev)
    func(a, c)
...
def test_cuda_floordiv_with_vectorization():
    with gsm_data_generator.target.cuda():
        # B[i] = A[floordiv(i, k)]
        n = 256
        k = 37
        A = te.placeholder((n,), name="A")
        B = te.compute((n,), lambda i: A[gsm_data_generator.tir.floordiv(i, k)], name="B")
        ...
        b_nd = gsm_data_generator.nd.array(np.zeros(b_np.shape, dtype=b_np.dtype), dev)
        func(a_nd, b_nd)
        gsm_data_generator.testing.assert_allclose(b_nd.numpy(), b_np, rtol=1e-3)
...
def test_cuda_floormod_with_vectorization():
    with gsm_data_generator.target.cuda():
        ...
        k = 37
        A = te.placeholder((n,), name="A")
        B = te.compute((n,), lambda i: A[gsm_data_generator.tir.floormod(i, k)], name="B")
        sch = gsm_data_generator.tir.Schedule(te.create_prim_func([A, B]))
        xo, xi = sch.split(sch.get_loops("B")[0], factors=[1, None])
        xio, xii = sch.split(xi, factors=[None, 4])
        sch.vectorize(xii)
...
def test_vectorized_casts():
    def check(t0, t1, factor):
        if (t0 == "float16" or t1 == "float16") and not have_fp16(gsm_data_generator.cuda(0).compute_version):
            print("Skip because gpu does not have fp16 support")
            return
        ...
        a_nd = gsm_data_generator.nd.array(a_np, dev)
        b_nd = gsm_data_generator.nd.array(b_np, dev)
        c_nd = gsm_data_generator.nd.array(np.zeros(c_np.shape, dtype=c_np.dtype), dev)
        func(a_nd, b_nd, c_nd)
        gsm_data_generator.testing.assert_allclose(c_nd.numpy(), c_np, rtol=1e-3)
...
def sched(A, B):
    ...
    return gsm_data_generator.compile(sch.mod, target="cuda")
...
def test_vectorized_intrin1():
    test_funcs = [
        (gsm_data_generator.tir.floor, lambda x: np.floor(x)),
        (gsm_data_generator.tir.ceil, lambda x: np.ceil(x)),
        (gsm_data_generator.tir.trunc, lambda x: np.trunc(x)),
        (gsm_data_generator.tir.abs, lambda x: np.fabs(x)),
        (gsm_data_generator.tir.round, lambda x: np.round(x)),
        (gsm_data_generator.tir.exp, lambda x: np.exp(x)),
        (gsm_data_generator.tir.exp2, lambda x: np.exp2(x)),
        (gsm_data_generator.tir.exp10, lambda x: np.power(10, x)),
        (gsm_data_generator.tir.log, lambda x: np.log(x)),
        ...
        (gsm_data_generator.tir.sqrt, lambda x: np.sqrt(x)),
    ]
    ...
    def run_test(tvm_intrin, np_func, dtype):
        ...
        skip_set = {
            gsm_data_generator.tir.abs,
            gsm_data_generator.tir.round,
            gsm_data_generator.tir.tan,
            gsm_data_generator.tir.atan,
            gsm_data_generator.tir.tanh,
            gsm_data_generator.tir.cosh,
            gsm_data_generator.tir.sinh,
        }
        ...
        b = gsm_data_generator.nd.array(np.zeros(shape=(n,)).astype(A.dtype), dev)
        f(a, b)
...
def test_vectorized_intrin2(dtype="float32"):
    c2 = gsm_data_generator.tir.const(2, dtype=dtype)
    test_funcs = [
        (gsm_data_generator.tir.power, lambda x: np.power(x, 2.0)),
        (gsm_data_generator.tir.fmod, lambda x: np.fmod(x, 2.0)),
    ]
...
def test_vectorized_popcount():
    ...
    def run_test(dtype):
        n = 128
        A = te.placeholder((n,), dtype=dtype, name="A")
        B = te.compute((n,), lambda i: gsm_data_generator.tir.popcount(A[i]), name="B")
        f = sched(A, B)
        dev = gsm_data_generator.cuda(0)
...
def test_cuda_vectorize_load_permute_pad():
    def check_cuda(dtype, n, l, padding, lanes):
        ...
        dev = gsm_data_generator.cuda(0)
        A = gsm_data_generator.te.placeholder((n, l), name="A", dtype=dtype)
        B = gsm_data_generator.te.compute(
            (n // lanes, l + 2 * padding, lanes),
            lambda i, j, k: gsm_data_generator.te.if_then_else(
                gsm_data_generator.te.any(j < padding, j >= l + padding),
                gsm_data_generator.tir.const(0, dtype),
        ...
        a = gsm_data_generator.nd.empty((n, l), A.dtype, dev).copyfrom(np_a)
        b = gsm_data_generator.nd.empty((n // lanes, l + padding * 2, lanes), B.dtype, dev)
        fun(a, b)
...
def test_try_unaligned_vector_load():
    ...
    def build(A, C, N, C_N):
        ...
        f = gsm_data_generator.tir.build(sch.mod, target="cuda")

        kernel_source = f.imported_modules[0].get_source()
        dev = gsm_data_generator.cuda()
        a_data = np.arange(0, N).astype(A.dtype)
        a = gsm_data_generator.nd.array(a_data, dev)
        c = gsm_data_generator.nd.array(np.zeros(C_N, dtype=C.dtype), dev)
        f(a, c)
...
def test_cuda_thread_sync_inside_condition():
    ...
    mod = gsm_data_generator.IRModule({"main": func1})
    with pytest.raises(gsm_data_generator.error.InternalError):
        gsm_data_generator.compile(mod, target="cuda")

    mod = gsm_data_generator.IRModule({"main": func2})
    gsm_data_generator.compile(mod, target="cuda")

    mod = gsm_data_generator.IRModule({"main": func3})
    gsm_data_generator.compile(mod, target="cuda")
...
def test_invalid_reinterpret():
    ...
    with pytest.raises(gsm_data_generator.error.TVMError):
        gsm_data_generator.compile(func, target="cuda")
...
def test_cuda_tensormap():
    ...
    mod = gsm_data_generator.IRModule({"main": main})
    mod = gsm_data_generator.compile(mod, target="cuda")
    assert (
        """
...
def test_cuda_device_func_call():
    ...
    lib = gsm_data_generator.compile(Module, target="cuda")
...
def test_device_host_call_same_func():
    ...
    target = gsm_data_generator.target.Target({"kind": "cuda", "mcpu": "dummy_mcpu"}, host="c")
    lib = gsm_data_generator.compile(Module, target=target)
    cuda_code = lib.mod.imported_modules[0].get_source()
    ...
    a_tvm = gsm_data_generator.nd.array(a_np, device=dev)
    b_tvm = gsm_data_generator.nd.array(b_np, device=dev)
    c_tvm = gsm_data_generator.nd.empty((128, 128), dtype="int32", device=dev)
    lib["main"](a_tvm, b_tvm, c_tvm)
    gsm_data_generator.testing.assert_allclose(c_tvm.numpy(), a_np + b_np)
