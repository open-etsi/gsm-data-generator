...
from gsmDataGen.contrib.nvcc import have_bf16, have_fp16, have_int8
...
def test_cuda_vectorize_add():
    ...
    def check_cuda(dtype, n, lanes):
        if dtype == "float16" and not have_fp16(gsmDataGen.cuda(0).compute_version):
            print("Skip because gpu does not have fp16 support")
            return
        if dtype == "int8" and not have_int8(gsmDataGen.cuda(0).compute_version):
        ...
        fun = gsmDataGen.compile(sch.mod, target="cuda")
...
def test_cuda_bf16_vectorize_add():
    if not have_bf16(gsmDataGen.cuda(0).compute_version):
    ...
    def check_cuda(n, lanes):
        ...
        a = gsmDataGen.nd.empty((n,), A.dtype, dev).copyfrom(np_float2np_bf16(np_a))
        c = gsmDataGen.nd.empty((n,), B.dtype, dev)
        fun(a, c)
        c = gsmDataGen.nd.empty((n, lanes), "uint16", dev).copyfrom(c)
        gsmDataGen.testing.assert_allclose(c.numpy(), np_float2np_bf16(np_a + 1))
...
def test_cuda_multiply_add():
    ...
    def check_cuda(dtype, n, lanes):
        ...
        D = te.compute(
            (n,), lambda i: gsmDataGen.tir.call_pure_extern("int32", "__dp4a", A[i], B[i], C[i]), name="D"
        )
        sch = gsmDataGen.tir.Schedule(te.create_prim_func([A, B, C, D]))
        ...
        dev = gsmDataGen.cuda(0)
        a = gsmDataGen.nd.empty((n,), A.dtype, dev).copyfrom(np_a)
        b = gsmDataGen.nd.empty((n,), B.dtype, dev).copyfrom(np_b)
        c = gsmDataGen.nd.empty((n,), C.dtype, dev).copyfrom(np_c)
        d = gsmDataGen.nd.empty((n,), D.dtype, dev)
        fun(a, b, c, d)
        gsmDataGen.testing.assert_allclose(d.numpy(), np_d)
...
def test_cuda_vectorize_load():
    ...
    def check_cuda(dtype, n, lanes):
        ...
        np_a = np.random.randint(low=-128, high=127, size=(n, lanes))
        a = gsmDataGen.nd.empty((n,), A.dtype, dev).copyfrom(np_a)
        b = gsmDataGen.nd.empty((n,), B.dtype, dev)
        fun(a, b)
        gsmDataGen.testing.assert_allclose(a.numpy(), b.numpy())
...
def test_cuda_make_int8():
    def check_cuda(n, value, lanes):
        dtype = "int8"
        dev = gsmDataGen.cuda(0)
        A = te.compute((n, lanes), lambda i, j: gsmDataGen.tir.const(value, dtype=dtype), name="A")

        sch = gsmDataGen.tir.Schedule(te.create_prim_func([A]))
        y, x = sch.get_loops("A")
        sch.vectorize(x)
        ...
        a = gsmDataGen.nd.empty(np_a.shape, dtype, dev)
        fun(a)
...
@gsmDataGen.testing.parametrize_targets("cuda", "rocm")
def test_crossthread_reduction1(target, dev):
    ...
    def verify(nthd):
        ...
        for kk in [x for x in vals]:
            size = (nn, kk)
            a = gsmDataGen.nd.array(np.random.uniform(size=size).astype(A.dtype), dev)
            b = gsmDataGen.nd.array(np.zeros(nn, dtype=B.dtype), dev)
            func(a, b)
            gsmDataGen.testing.assert_allclose(b.numpy(), np.sum(a.numpy(), axis=1), rtol=1e-3)
...
def test_crossthread_reduction2(target, dev):
    ...
    def sched(nthdx, nthdy):
        ...
        sch.bind(x, "blockIdx.x")
        func = gsmDataGen.compile(sch.mod, target="cuda")
        return func
    ...
    def verify(nthdx, nthdy):
        ...
        for kk0, kk1 in [(x, y) for x in vx for y in vy]:
            ...
            b = gsmDataGen.nd.array(np.zeros(nn, dtype=B.dtype), dev)
            func(a, b)
            gsmDataGen.testing.assert_allclose(b.numpy(), np.sum(a.numpy(), axis=(1, 2)), rtol=1e-3)
...
def test_cuda_const_float_to_half():
    ...
    b = gsmDataGen.tir.const(0.5, dtype="float16")
    ...
    c = gsmDataGen.nd.array(c_np, dev)
    func(a, c)
...
def test_cuda_floordiv_with_vectorization():
    with gsmDataGen.target.cuda():
        # B[i] = A[floordiv(i, k)]
        n = 256
        k = 37
        A = te.placeholder((n,), name="A")
        B = te.compute((n,), lambda i: A[gsmDataGen.tir.floordiv(i, k)], name="B")
        ...
        a_nd = gsmDataGen.nd.array(a_np, dev)
        b_nd = gsmDataGen.nd.array(np.zeros(b_np.shape, dtype=b_np.dtype), dev)
        func(a_nd, b_nd)
        gsmDataGen.testing.assert_allclose(b_nd.numpy(), b_np, rtol=1e-3)
...
def test_cuda_floormod_with_vectorization():
    with gsmDataGen.target.cuda():
        ...
        k = 37
        A = te.placeholder((n,), name="A")
        B = te.compute((n,), lambda i: A[gsmDataGen.tir.floormod(i, k)], name="B")
        sch = gsmDataGen.tir.Schedule(te.create_prim_func([A, B]))
        xo, xi = sch.split(sch.get_loops("B")[0], factors=[1, None])
        xio, xii = sch.split(xi, factors=[None, 4])
        sch.vectorize(xii)
...
def test_vectorized_casts():
    def check(t0, t1, factor):
        if (t0 == "float16" or t1 == "float16") and not have_fp16(gsmDataGen.cuda(0).compute_version):
            print("Skip because gpu does not have fp16 support")
            return
        ...
        a_nd = gsmDataGen.nd.array(a_np, dev)
        b_nd = gsmDataGen.nd.array(b_np, dev)
        c_nd = gsmDataGen.nd.array(np.zeros(c_np.shape, dtype=c_np.dtype), dev)
        func(a_nd, b_nd, c_nd)
        gsmDataGen.testing.assert_allclose(c_nd.numpy(), c_np, rtol=1e-3)
...
def sched(A, B):
    ...
    return gsmDataGen.compile(sch.mod, target="cuda")
...
def test_vectorized_intrin1():
    test_funcs = [
        (gsmDataGen.tir.floor, lambda x: np.floor(x)),
        (gsmDataGen.tir.ceil, lambda x: np.ceil(x)),
        (gsmDataGen.tir.trunc, lambda x: np.trunc(x)),
        (gsmDataGen.tir.abs, lambda x: np.fabs(x)),
        (gsmDataGen.tir.round, lambda x: np.round(x)),
        (gsmDataGen.tir.exp, lambda x: np.exp(x)),
        (gsmDataGen.tir.exp2, lambda x: np.exp2(x)),
        (gsmDataGen.tir.exp10, lambda x: np.power(10, x)),
        (gsmDataGen.tir.log, lambda x: np.log(x)),
        ...
        (gsmDataGen.tir.sqrt, lambda x: np.sqrt(x)),
    ]
    ...
    def run_test(tvm_intrin, np_func, dtype):
        ...
        b = gsmDataGen.nd.array(np.zeros(shape=(n,)).astype(A.dtype), dev)
        f(a, b)
...
def test_vectorized_intrin2(dtype="float32"):
    c2 = gsmDataGen.tir.const(2, dtype=dtype)
    test_funcs = [
        (gsmDataGen.tir.power, lambda x: np.power(x, 2.0)),
        (gsmDataGen.tir.fmod, lambda x: np.fmod(x, 2.0)),
    ]
...
def test_vectorized_popcount():
    ...
    def run_test(dtype):
        n = 128
        A = te.placeholder((n,), dtype=dtype, name="A")
        B = te.compute((n,), lambda i: gsmDataGen.tir.popcount(A[i]), name="B")
        f = sched(A, B)
        dev = gsmDataGen.cuda(0)
...
def test_cuda_vectorize_load_permute_pad():
    def check_cuda(dtype, n, l, padding, lanes):
        ...
        dev = gsmDataGen.cuda(0)
        A = gsmDataGen.te.placeholder((n, l), name="A", dtype=dtype)
        B = gsmDataGen.te.compute(
            (n // lanes, l + 2 * padding, lanes),
            lambda i, j, k: gsmDataGen.te.if_then_else(
                gsmDataGen.te.any(j < padding, j >= l + padding),
        ...
        a = gsmDataGen.nd.empty((n, l), A.dtype, dev).copyfrom(np_a)
        b = gsmDataGen.nd.empty((n // lanes, l + padding * 2, lanes), B.dtype, dev)
        fun(a, b)
...
def test_try_unaligned_vector_load():
    ...
    def build(A, C, N, C_N):
        ...
        f = gsmDataGen.tir.build(sch.mod, target="cuda")

        kernel_source = f.imported_modules[0].get_source()
        dev = gsmDataGen.cuda()
        a_data = np.arange(0, N).astype(A.dtype)
        a = gsmDataGen.nd.array(a_data, dev)
        c = gsmDataGen.nd.array(np.zeros(C_N, dtype=C.dtype), dev)
        f(a, c)
...
def test_cuda_thread_sync_inside_condition():
    ...
    mod = gsmDataGen.IRModule({"main": func1})
    with pytest.raises(gsmDataGen.error.InternalError):
        gsmDataGen.compile(mod, target="cuda")

    mod = gsmDataGen.IRModule({"main": func2})
    gsmDataGen.compile(mod, target="cuda")

    mod = gsmDataGen.IRModule({"main": func3})
    gsmDataGen.compile(mod, target="cuda")
...
def test_invalid_reinterpret():
    ...
    with pytest.raises(gsmDataGen.error.TVMError):
        gsmDataGen.compile(func, target="cuda")
...
def test_cuda_tensormap():
    ...
    mod = gsmDataGen.IRModule({"main": main})
    mod = gsmDataGen.compile(mod, target="cuda")
    assert (
        """
...
def test_cuda_device_func_call():
    ...
    lib = gsmDataGen.compile(Module, target="cuda")
...
def test_device_host_call_same_func():
    ...
    target = gsmDataGen.target.Target({"kind": "cuda", "mcpu": "dummy_mcpu"}, host="c")
    lib = gsmDataGen.compile(Module, target=target)
    cuda_code = lib.mod.imported_modules[0].get_source()
    ...
    a_tvm = gsmDataGen.nd.array(a_np, device=dev)
    b_tvm = gsmDataGen.nd.array(b_np, device=dev)
    c_tvm = gsmDataGen.nd.empty((128, 128), dtype="int32", device=dev)
    lib["main"](a_tvm, b_tvm, c_tvm)
    gsmDataGen.testing.assert_allclose(c_tvm.numpy(), a_np + b_np)
