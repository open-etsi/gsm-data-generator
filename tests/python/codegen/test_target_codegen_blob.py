...
import ctypes
import numpy as np
from gsmDataGen.contrib import cc, utils, popen_pool, tar
import gsmDataGen
...
def test_cuda_multi_lib():
    ...
    libA = gsmDataGen.compile(ModA, target=target)
    libB = gsmDataGen.compile(ModB, target=target)
    ...
    def popen_check():
        ...
        b_nd = gsmDataGen.nd.array(a_np, dev)
        syslibA = gsmDataGen.runtime.system_lib("modA_")
        syslibB = gsmDataGen.runtime.system_lib("modB_")
