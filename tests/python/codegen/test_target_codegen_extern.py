...
def test_add_pipeline():
    ...
    def extern_generator(ins, outs):
        ...
        with ib.for_range(0, (n + 1) // 2) as i:
            ib.emit(
                outs[0].vstore(
                    i * 2, ins[0].vload(i * 2, "float32x2") + gsmDataGen.tir.const(1, "float32x2")
                )
            )
        return ib.get()
    ...
    def extern_generator_gpu(ins, outs):
        ...
        with ib.if_scope(ib.likely(idx < n)):
            ib.emit(
                outs[0].vstore(
                    idx * 2, ins[0].vload(idx * 2, "float32x2") + gsmDataGen.tir.const(1, "float32x2")
                )
            )
        return ib.get()
    ...
    mod_cpu = gsmDataGen.IRModule.from_expr(te.create_prim_func([A, C_cpu]))
    mod_gpu = gsmDataGen.IRModule.from_expr(te.create_prim_func([A, C_gpu]))
...
def test_pack_buffer_simple():
    ...
    def extern_generator(ins, outs):
        """Manually write the IR for the extern function, add pipeline."""
        return gsmDataGen.tir.call_packed("my_extern_array_func1", ins[0], outs[0])

    C = te.extern(A.shape, [A], extern_generator, name="C")

    # Create IRModule directly
    mod = gsmDataGen.IRModule.from_expr(te.create_prim_func([A, C]))
    ...
    def check_target(target):
        ...
        f(a, c)
        gsmDataGen.testing.assert_allclose(c.numpy(), a.numpy())
...
def test_pack_buffer_intermediate():
    ...
    def extern_generator(ins, outs):
        """Manually write the IR for the extern function, add pipeline."""
        return gsmDataGen.tir.call_packed("my_extern_array_func2", ins[0], outs[0])
    ...
    def check_target(target):
        ...
        def my_extern_array_func2(aa, bb):
            ...
            gsmDataGen.testing.assert_allclose(aa.numpy(), a.numpy() + 1)
            aa.copyto(bb)
