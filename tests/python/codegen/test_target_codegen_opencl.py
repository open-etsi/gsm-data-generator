...
@gsm_data_generator.testing.requires_opencl
def test_opencl_ternary_expression():
    def check_if_then_else(dev, n, dtype):
        A = te.placeholder((n,), name="A", dtype=dtype)
        true_value = gsm_data_generator.tir.const(1, dtype=dtype)
        false_value = gsm_data_generator.tir.const(3, dtype=dtype)
        max_lhs = gsm_data_generator.tir.const(2, dtype=dtype)
        max_rhs = gsm_data_generator.tir.if_then_else(A[0] > 0, true_value, false_value)
        C = te.compute((n,), lambda i: gsm_data_generator.te.max(max_lhs, max_rhs), name="C")

        func = te.create_prim_func([A, C])
        sch = gsm_data_generator.tir.Schedule(func)
        (x,) = sch.get_loops(sch.get_block("C"))
        sch.bind(x, "threadIdx.x")
        fun = gsm_data_generator.tir.build(sch.mod, target=target)
    ...
    def check_select(dev, n, dtype):
        ...
        false_value = gsm_data_generator.tir.const(3, dtype=dtype)
        max_lhs = gsm_data_generator.tir.const(2, dtype=dtype)
        max_rhs = gsm_data_generator.tir.Select(A[0] > 0, true_value, false_value)
        C = te.compute((n,), lambda i: gsm_data_generator.te.max(max_lhs, max_rhs), name="C")
        func = te.create_prim_func([A, C])
        sch = gsm_data_generator.tir.Schedule(func)
        (x,) = sch.get_loops(sch.get_block("C"))
...
def test_opencl_max():
    def check_max(dev, n, dtype):
        A = te.placeholder((n,), name="A", dtype=dtype)
        max_lhs = A[0] + gsm_data_generator.tir.const(1, dtype=dtype)
        max_rhs = gsm_data_generator.tir.const(0, dtype=dtype)
        C = te.compute((n,), lambda i: gsm_data_generator.te.max(max_lhs, max_rhs), name="C")
        func = te.create_prim_func([A, C])
        sch = gsm_data_generator.tir.Schedule(func)
        (x,) = sch.get_loops(sch.get_block("C"))
...
def test_opencl_type_casting():
    def check_type_casting(ctx, n, dtype):
        ...
        C = te.compute(
            ...
            lambda i: gsm_data_generator.tir.Select(
                gsm_data_generator.tir.all(
                    *[
                        i // block_size == gsm_data_generator.tir.const(3, "int32"),
                        i % 3 == gsm_data_generator.tir.const(1, "int32"),
                    ]
                ),
                gsm_data_generator.tir.const(1, dtype),
        ...
        fun = gsm_data_generator.tir.build(sch.mod, target=target)
        c = gsm_data_generator.nd.empty((n,), dtype, ctx)
...
@gsm_data_generator.testing.requires_gpu
@gsm_data_generator.testing.requires_opencl
@gsm_data_generator.testing.parametrize_targets("opencl", "opencl -device=adreno")
def test_opencl_ceil_log2(target):
    def _check(target, n, dtype):
        with gsm_data_generator.target.Target(target):
            C = te.compute(
                (n,),
                lambda i: gsm_data_generator.topi.ceil_log2(i),
                name="C",
            )
            func = te.create_prim_func([C])
            sch = gsm_data_generator.tir.Schedule(func)
