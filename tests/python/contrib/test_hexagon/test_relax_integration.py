...
from gsmDataGen import relax, runtime
from gsmDataGen.relax.frontend import onnx
from gsmDataGen.relax.testing import relay_translator
...
def get_onnx_mobilenet():
    ...
    model_path = gsmDataGen.contrib.download.download_testdata(
...
def test_mobilenet_onnx(hexagon_session: Session):
    ...
    target_hexagon = gsmDataGen.target.hexagon("v68")
    ...
    exe = gsmDataGen.compile(relax_mod, "llvm")
    dev = gsmDataGen.cpu()
    vm_rt = relax.VirtualMachine(exe, dev)
    data = gsmDataGen.nd.array(data_np, dev)
    llvm_res = vm_rt["main"](data)
    gsmDataGen.testing.assert_allclose(hexagon_res.numpy(), llvm_res.numpy(), rtol=1e-3)
